...
- A profiler-style SWARD operator panel is now the default visible operator surface: one `DrawProfiler()`-like window with a frame-time graph and Runtime/HUD/Capture/Panels tabs. The older counter/view/export/debug-draw windows remain focused drill-downs from the Panels tab, the live bridge/API and debug-menu-fork typed fields stay enabled by default, and F1 remains reserved for the native Recomp Profiler instead of being captured by SWARD.
