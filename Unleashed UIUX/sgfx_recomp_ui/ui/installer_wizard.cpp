// DECOUPLED from UnleashedRecomp/ui/installer_wizard.cpp. The game-runtime includes
// are replaced by the lib's shim + render contract; the UI helpers (imgui_utils,
// button_guide) and the sibling message_window stay. Everything else — the whole
// wizard body, animations, NFD picker plumbing, page/cursor logic — is the recomp's
// code byte-for-byte (Config::/g_aspectRatio*/Localise/Scale/ImFontAtlasSnapshot
// resolve through the shim under their original names).
//
// This is the most game-coupled file in the set: it owns the install pipeline
// (Installer::), embedded music (EmbeddedPlayer::), the SDL event listener, the
// app/window loop (GameWindow::/Video::), input (hid::), the sound cue hook
// (Game_PlaySound) and credits. Those are NOT pure UI — they are reported in
// gameStateCoupling and the symbols the shim must add are reported in newShimSymbols.
// The 12 install/icon sprites load via render::LoadUISprite (reported).
...
static std::array<std::unique_ptr<sgfx::render::Texture>, 8> g_installTextures;
static std::unique_ptr<sgfx::render::Texture> g_milesElectricIcon;
static std::unique_ptr<sgfx::render::Texture> g_arrowCircle;
static std::unique_ptr<sgfx::render::Texture> g_pulseInstall;
static std::unique_ptr<sgfx::render::Texture> g_upHedgeDev;
...
static void DrawLeftImage()
{
    ...
    sgfx::render::Texture *guestTexture = g_installTextures[installTextureIndex % g_installTextures.size()].get();
    auto &res = ImGui::GetIO().DisplaySize;
    auto drawList = ImGui::GetBackgroundDrawList();
...
void InstallerWizard::Init()
{
    ...
    g_installTextures[0] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install001);
    g_installTextures[1] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install002);
    g_installTextures[2] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install003);
    g_installTextures[3] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install004);
    g_installTextures[4] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install005);
    g_installTextures[5] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install006);
    g_installTextures[6] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install007);
    g_installTextures[7] = sgfx::render::LoadUISprite(sgfx::render::UISprite::Install008);
    g_milesElectricIcon = sgfx::render::LoadUISprite(sgfx::render::UISprite::MilesElectricIcon);
    g_arrowCircle = sgfx::render::LoadUISprite(sgfx::render::UISprite::ArrowCircle);
    g_pulseInstall = sgfx::render::LoadUISprite(sgfx::render::UISprite::PulseInstall);
    g_upHedgeDev = sgfx::render::LoadUISprite(sgfx::render::UISprite::HedgeDev);
