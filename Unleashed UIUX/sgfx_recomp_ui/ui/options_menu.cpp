// DECOUPLED from UnleashedRecomp/ui/options_menu.cpp. The deepest game-coupled menu:
// the full Config/ConfigDef<T> options system, SWA game input (CInputState), Game_PlaySound,
// GameWindow display-mode queries, App/Video runtime state, AudioPatches, the HUD-render flag
// and an SDL right-stick listener. All game-runtime #includes below were swapped for the shim
// + render contract; imgui_utils (UI helpers), options_menu_thumbnails and tv_static stay.
// Symbols that aren't yet in the shim keep their EXACT recomp names (Config::*, SWA::*,
// hid::*, App::*, Video::*, GameWindow::*, AudioPatches::*, Game_PlaySound, the E* config
// enums, g_aspectRatioNarrowScale, ...) and are reported for central merge; body byte-identical.
...
#include <memory>
#include <string>
#include <fmt/format.h>   // the WindowSize/Monitor/FPS value-text fmt::format calls (was transitive via stdafx)

// The Miles-Electric foreground sprite (recomp res/images/options_menu/miles_electric.dds)
// now loads through the render contract; see OptionsMenu::Init.
...
static std::unique_ptr<sgfx::render::Texture> g_upMilesElectric;
...
void OptionsMenu::Init()
{
    ...
    g_upMilesElectric = sgfx::render::LoadUISprite(sgfx::render::UISprite::OptionsMilesElectric);
