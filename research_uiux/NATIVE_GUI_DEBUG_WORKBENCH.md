...
- It now emits authored CSD cast transform, keyframe curve, keyframe sample, sampled transform, sampled draw-command, and sampled channel-state descriptors for a first exact-family sample set and draws those sampled transform markers in the preview, but it still does not drive full CSD animation playback.
