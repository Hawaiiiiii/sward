...
- The native Recomp Profiler is the primary operator surface for UI Lab sessions: Phase 184 embeds compact SWARD UI Lab readouts, HUD value binding status, capture state, and the key HUD SGlobals directly into the existing `DrawProfiler()` panel. The older counter/view/export/debug-draw windows remain opt-in Legacy floating panes, the live bridge/API and debug-menu-fork typed fields stay enabled by default, and F1 remains reserved for the native Recomp Profiler instead of being captured by SWARD.
