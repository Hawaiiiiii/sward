...
- a Reddog-inspired SWARD operator shell with an F1-toggled operator shell, draggable debug icon, window list, counter/view/export/debug-draw windows, and a foreground debug-draw layer for live lab state. This ports the useful operator workflow from the local fork without importing its source/assets or replacing SWARD's target routing and evidence oracle.
- Compact-on-demand operator windows for the useful debug-menu-fork workflow: the live bridge/API and debug-menu-fork typed fields stay enabled by default, but welcome/status, inspector, counters, view, exports, debug draw, Stage/HUD operator, live API panes, and the foreground operator layer now open only when requested from the small `UI` button. This keeps manual gameplay sessions readable and avoids paying ImGui/window-list cost unless we are actively inspecting.
...
- A visible operator-shell smoke under `out/ui_lab_runtime_evidence/20260427_194221/` passed on the older default-open build and wrote `ui_lab_live_state.json` beside the screenshot evidence. Phase 178 supersedes that visible default: operator panes are now compact-on-demand while the live bridge remains the primary direct-read path.
