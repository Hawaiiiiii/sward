...
- `research_uiux/runtime_reference/examples/su_ui_asset_renderer.cpp`
  ...
  - Phase 191 begins consuming those stable semantic candidates without pretending exact child-node resolution is complete. The runtime now emits `sonic-hud-node-write-semantic-bound` when a `generated-PPC-callsite-semantic-candidate` can safely feed typed Sonic HUD gameplay values, with `pathResolved=false` and `semanticBindingStatus=stable-candidate-bound-pending-exact-child-node-resolution` (`semantic-candidate-bound-pending-exact-child-node-resolution` in controller smoke output). `SonicDayHudController::applyRuntimeSemanticPathCandidate` mirrors that payload in reusable source, binding candidate-backed speed/boost/ring-energy/tutorial state while keeping the final `ui_playscreen/...` child/cast/node resolver as the next hard step.
  - Phase 192 promotes the Phase 191 manual gameplay proof into the summarizer without blurring evidence classes. The tool now counts `sonic-hud-node-write-semantic-bound` separately as `semantic_bound`, reports `semantic_bound_paths`, and groups bound runtime-fed values as `semantic_bound_groups`, so a semantic-bound tutorial lane (`ui_playscreen/add/u_info:tutorialPrompt`) is visibly stronger than a raw candidate while still separate from exact `pathResolved=true` child-node recovery.
  - Phase 193 applies that split to reusable controller source. The Phase 192 manual gameplay window proved semantic-bound `elapsedFrames`, `speedKmh`, and `tutorialPrompt` lanes, while boost/ring-energy remained high-volume `semantic_path_candidate` groups only. `SonicDayHudRuntimeSemanticPathCandidateObservation` now carries `bindingStatus`, and `sward_frontend_screen_controller_catalog --phase193-sonic-hud-semantic-bound-smoke` applies only the bound lanes to a strict controller state. This prevents the reusable source lane from treating boost/ring-energy as 1:1 runtime-bound before exact `ui_playscreen/...` child/cast/node resolution proves them.
  - Phase 194 uses live `ui-draw-list` evidence to name the visible boost/ring-energy gauge draw child paths without pretending the SetScale/SetPatternIndex/SetHideFlag setter nodes are joined yet. `summarize_unleashed_recomp_ui_lab_hud_values.ps1 -DrawListPath ...` now reports gauge draw child paths as `gauge_draw_path_groups`, keeps setter candidates in `gauge_setter_node_candidates`, and emits `gauge_child_path_status=runtime-draw-list-exact-child-paths;setter-node-address-join-pending`. Current exact draw parents are `ui_playscreen/so_speed_gauge/position/speed_gauge_color` and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color`; the controller smoke path records those as `exact-runtime-draw-child-paths-known` while keeping controller value updates blocked on `setter-node-address-join-pending`.
  - Phase 195 adds the strict setter-node address join on top of that draw-list evidence. A boost/ring-energy setter now becomes `runtime-draw-list-setter-node-joined` only if the unresolved CSD setter node address matches a live draw-list `castNodeAddress` or `layerAddress`, the semantic value name matches the gauge, and the semantic path candidate prefixes the exact gauge fill parent. The summarizer emits `gauge_setter_child_path_joins`, and controller source adds `SonicDayHudRuntimeGaugeSetterChildPathJoin` so joined evidence such as `ui_playscreen/so_speed_gauge/position/speed_gauge_color/Cast_0506` and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color/Cast_0483` can drive `boostGauge` / `ringEnergyGauge` as `setter-node-address-join-runtime-proven` inputs. This still does not solve exact SFX/audio IDs.
  - Phase 196 follows the actual manual gameplay evidence around `sub_824D6C18` instead of forcing the missing setter-node join. The live run showed high-volume `kind=text` boost/ring-energy candidates from the rolling counter/update callsite while the visible gauge fill draw paths were already known and `gauge_setter_child_path_joins` remained empty. The summarizer now groups those as `rolling_counter_semantic_groups` and emits `boost_ring_energy_status=rolling-counter-text-candidate-pending-gauge-state-normalization`; controller source adds `SonicDayHudRuntimeRollingGaugeCounterObservation` so reusable HUD code can report this stronger candidate lane without treating it as final boost/ring-energy gauge state until the exact child/gauge-state normalization is proven. In plain terms, boost/ring-energy now have pending gauge-state normalization rather than a fake direct setter proof.
  - Phase 197 traces `sub_824D6C18` one level deeper without overclaiming what each owner-field offset means, opening a parallel owner-field rolling counter lane so the Phase 196 text-write candidate is no longer the only evidence channel. The runtime patch hook on `sub_824D6C18` now snapshots the `CHudSonicStage` owner-field dwords at `+460/+464/+468/+472/+480` after the original PPC implementation runs, gated by the existing 600-frame stable-signature throttle so the FPS profile stays flat under manual gameplay; the snapshot rides as a new `sonic-hud-owner-gauge-snapshot` evidence event. Evidence-backed today: an owner-field rolling counter signal that co-occurs with the same-frame boost and ring-energy text candidates at the `sub_824D6C18` callsite, surfaced by the summarizer as `owner_field_rolling_counter_groups=owner=...:field+N=valueName:samples=M` and `owner_field_rolling_counter_status=owner-field-rolling-counter-pending-exact-offset-normalization`, plus reusable controller source `SonicDayHudRuntimeOwnerFieldRollingCounterObservation` / `applyRuntimeOwnerFieldRollingCounterObservation` and a `--phase197-sonic-hud-owner-field-rolling-counter-smoke` smoke that mutates only `valueSource` provenance on `boostGauge` and `ringEnergyGauge`. Still pending: the exact per-offset semantic (which of `+460/+464/+468/+472/+480` is the integer rolling counter vs. the float fill scale vs. the hide/style enum vs. the animation phase index), the `gauge_setter_child_path_joins` lane that remains empty, and any 1:1 boost/ring-energy gauge final value — the `boost_ring_energy_status` lane is intentionally kept independent from `owner_field_rolling_counter_status` until both correlate. In plain terms, Phase 197 makes boost/ring-energy stronger as candidate evidence at the owner-field level without faking a 1:1 boost/energy reading. Live verification on `2026-05-01` against `out/ui_lab_runtime_evidence/phase197_manual_20260501_013619` recovered live owner `0xDE94C30` and 8 throttled snapshots showing `+460=4, +464=fluctuating(0..8), +468=fluctuating(0..7), +472=fluctuating(0..9), +480=non-monotonic(0..895)`, consistent with `+480` as a wrapping accumulator and the others as small-int phase/style indices, but per-offset semantic stays pending until Phase 198 same-frame setter-scale correlation proves it.
  - Phase 198 opens the owner-field-to-setter-scale correlation lane to start disambiguating which Phase 197 offset is the integer rolling counter vs. the float fill scale vs. the hide/style enum vs. the animation phase index, by joining each owner-field snapshot with the same-frame `CSD::CNode::SetScale` write on the proven gauge-fill cast nodes `ui_playscreen/so_speed_gauge/position/speed_gauge_color/Cast_0506` (boost) and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color/Cast_0483` (ring-energy). The runtime caches the latest five owner-field dwords plus the owner address inside an `OwnerFieldGaugeSnapshotCache` mutex-protected struct after every `OnHudSonicStageOwnerFieldGaugeSnapshot` call (independent of the Phase 197 emission throttle so the cache is always fresh), then `OnCsdNodeSetScale` reads the cache when its already-resolved `ui_playscreen/so_*_gauge/...` path indicates a gauge-fill setter and emits a new `sonic-hud-gauge-scale-owner-correlated` evidence event keyed to `(path, scale, ownerSignature)` so it does not flood the JSONL. The summarizer reports `owner_field_gauge_scale_correlation_groups=path=...:owner=...:field+N:joins=M` and `owner_field_gauge_scale_correlation_status=owner-field-gauge-scale-correlation-pending-formula-proof` — kept independent from `boost_ring_energy_status` and `owner_field_rolling_counter_status` so each evidence channel stays separately auditable. Reusable controller source adds `SonicDayHudRuntimeOwnerFieldGaugeScaleCorrelation` / `applyRuntimeOwnerFieldGaugeScaleCorrelation` and a `--phase198-sonic-hud-owner-field-gauge-scale-correlation-smoke` that mutates only `valueSource` provenance on `boostGauge` and `ringEnergyGauge`. Evidence-backed today: a same-frame join between a Phase 197 owner-field snapshot and a Phase 195 visible gauge-fill setter, with the correlation expressed in the controller smoke as `field+N -> Cast_0506/Cast_0483` candidate pairs. Still pending: the exact per-offset formula (linear normalize, fixed-point Q-format, modulo-wrapped accumulator, or piecewise lookup), the `gauge_setter_child_path_joins` lane that remains empty, and any 1:1 boost/ring-energy gauge final value. In plain terms, Phase 198 starts mapping Phase 197 evidence onto Phase 195 visible geometry without faking a 1:1 boost/energy reading.
  - Phase 199 adds an honest owner-field offset classification lane on top of the Phase 197/198 evidence already in `ui_lab_events.jsonl`, without any new runtime hook and without claiming a formula. The summarizer aggregates per-offset observed integer values across both gauge `valueName`s of Phase 197 owner-gauge snapshots and the join count from Phase 198 same-frame SetScale correlations, then `Resolve-OwnerFieldOffsetCandidateLabel` tags each `CHudSonicStage` owner-field offset with one of five *shape* labels: `low-cardinality-narrow-range-candidate` (likely hide flag / style enum), `moderate-cardinality-narrow-range-candidate` (likely animation phase index), `high-cardinality-narrow-range-candidate` (likely fixed-point fill scale), `high-cardinality-wide-range-candidate` (likely rolling counter / wrap accumulator), or `unclassified-pending-more-evidence`. The summarizer reports `owner_field_offset_classifications=owner=...:field+N:cardinality=C:min=Lo:max=Hi:joins=J:candidate=label` and `owner_field_offset_classification_status=owner-field-offset-classification-pending-formula-proof`, intentionally kept independent from `boost_ring_energy_status`, `owner_field_rolling_counter_status`, and `owner_field_gauge_scale_correlation_status` so each evidence channel stays separately auditable. Reusable controller source adds `SonicDayHudRuntimeOwnerFieldOffsetClassification` / `applyRuntimeOwnerFieldOffsetClassification` and `formatSonicDayHudRuntimeBindingPhase199SmokeSequence` (`--phase199-sonic-hud-owner-field-offset-classification-smoke`); the smoke output writes one classification line per offset and mutates only `valueSource` provenance — `gameplayState_.boostGauge` / `ringEnergyGauge` stay at controller defaults because *shape labels are not formulas*. Evidence-backed today: a per-offset shape candidate label derived only from already-captured Phase 197/198 evidence. Still pending: the exact per-offset formula, the `gauge_setter_child_path_joins` lane that remains empty, the `CSD::CNode::SetPatternIndex` / `CSD::CNode::SetHideFlag` join extension for the hide/style enum candidate offset, and any 1:1 boost/ring-energy gauge final value. In plain terms, Phase 199 attaches honest shape names to each owner-field offset so future runtime evidence can be reasoned about against named candidates without inventing a formula yet.
  - Phase 200 tested whether the existing Phase 197/198/199 evidence in `ui_lab_events.jsonl` can prove a formula mapping `CHudSonicStage` owner-field offsets `+460/+464/+468/+472/+480` to the boost / ring-energy gauge fill scales on `Cast_0506` / `Cast_0483`, and added an owner-field offset transition diagnostics lane to surface what the Phase 197 snapshot stream can and cannot say on its own. The answer from the latest capture is **no formula proven across multiple frames**: the latest manual session stayed on the title loop, the constructor-seam owner-field path produced 26 throttled `sonic-hud-owner-gauge-snapshot` events with a real live owner `0xDE94C30`, but the same session emitted zero `sonic-hud-gauge-scale-write` and zero `sonic-hud-gauge-scale-owner-correlated` events on the boost / ring-energy gauge fill cast nodes, so neither offset can be correlated to `Cast_0506` or `Cast_0483` from this evidence. Candidate formulas tracked for the next stage-gameplay capture: `+N` interpreted as IEEE float, `+N / 16` (Q.4 fixed-point) and `+N / 256` (Q.8 fixed-point) linear normalizers, modulo-wrapped accumulators, and inter-offset coupling (e.g. `floor(+480 / N)`); none of them is testable from a session that never reached the Sonic Day stage. Per the no-fake-1:1 rule, no controller value was promoted: `gameplayState_.boostGauge` / `ringEnergyGauge` stay at controller defaults; `boost_ring_energy_status`, `owner_field_rolling_counter_status`, `owner_field_gauge_scale_correlation_status`, and `owner_field_offset_classification_status` all remain pending. Concrete summarizer-only diagnostic added: per (`ownerAddress`, `fieldOffset`) the consecutive-pair transition statistics across the snapshot timeline are computed in chronological order and reported as `owner_field_offset_transition_diagnostics=owner=...:field+N:trend=stable|increasing|decreasing|non-monotonic|no-transitions:wrap=true|false:transitions=T:inc=I:dec=D:eq=E:min_delta=M:max_delta=X`, plus a parallel `owner_field_offset_transition_diagnostics_status=owner-field-offset-transition-diagnostics-pending-formula-proof` so future captures can be triaged without first eyeballing the JSONL. New PowerShell helpers `New-OwnerFieldOffsetTransitionTrack`, `Add-OwnerFieldOffsetTransitionSample`, `Resolve-OwnerFieldOffsetMonotonicTrendLabel` preserve the temporal order that the existing `New-OwnerFieldRollingCounterGroup` SortedSet discards, so wrap detection (any decreasing transition) and shape mix (inc / dec / eq counts) read straight off the summary. Live cross-check on the latest capture (26 snapshots, owner `0xDE94C30`): `+460` shows three distinct values `{0, 2, 4}` with both increasing and decreasing transitions (state-flag-like, wrap detected only because the 0/4 toggle counts as a decrease), `+464`, `+468`, `+472` stay bounded in `[0, 9]` with mixed transitions (consistent with mod-`N` indices, `N <= 16`), and `+480` is the only wide-range non-monotonic offset (15 distinct values in `[0, 1494]`, multiple decreases, consistent with a wrap accumulator) — no runtime hook change, no controller struct change, and no formula promotion until a stage-gameplay capture under the Phase 198 build emits same-frame SetScale joins on `Cast_0506` / `Cast_0483`.
  - Phase 201 adds an owner-field-to-visible-gauge-path bridge for the current manual stage evidence without inventing a formula. The Phase 201 live run produced real `sub_824D6C18` owner-field samples and a live `ui-draw-list` snapshot where the exact visible gauge fill children are present under `ui_playscreen/so_speed_gauge/position/speed_gauge_color/Cast_0506` and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color/Cast_0483`; however, the same run still produced zero `sonic-hud-gauge-scale-write` and zero `sonic-hud-gauge-scale-owner-correlated` events. The summarizer now records this middle state with `owner_field_draw_path_bridge_groups=path=...:value=...:owner=...:field+N:samples=S:draws=D` and `owner_field_draw_path_bridge_status=owner-field-draw-path-bridge-pending-formula-proof`. Evidence-backed today: the owner-field rolling-counter lane can be attached to exact visible gauge draw paths by semantic value. Still pending: parent/group-node setter resolution, exact per-offset formula, and final 1:1 boost/ring-energy value promotion.
  - Phase 202 adds a preview build secondary oracle lane while keeping the retail UnleashedRecomp runtime as the primary oracle: do not replace the retail runtime oracle. The local preview build is useful because it carries debug-oriented route/menu material and earlier UI packages, but it must not replace the retail runtime proof path or be published. The new `inventory_sonic_unleashed_preview_build.ps1` tool emits metadata-only inventory for a local preview root: XEX/shader presence, `.ar.00` archive families, UI-relevant archive families, localized archive families, sound-bank names, expected UI package names, and Reddog debug UI assets. Local-only extraction with the supplied toolbox proved `ui_title`, E3 `ui_title`, `ui_mainmenu`, `ui_loading`, `ui_worldmap`, `ActionCommon/ui_playscreen`, `EvilActionCommon/ui_playscreen_evil`, `EvilActionCommon/ui_playscreen_ev_hit`, and `ActionCommon/ui_result`; `#SelectStage/Select.xml` exposes route categories such as `Title`, `WorldMap`, `OldMainMenu`, `InstallTest`, `SaveTest`, and `SequenceEntryPoint`. Evidence-backed today: the preview is a strong secondary oracle for menu route taxonomy, Reddog debug-window styling, early HUD package comparison, and sound/localization bank discovery. Still pending: any isolated prototype recomp build, any verified address/hook transfer into retail, and any claim that preview behavior is 1:1 retail behavior.
  - Phase 203 turns the extracted prototype `#SelectStage/Select.xml` into a repo-safe route taxonomy instead of trying to run the prototype executable inside the retail recomp lane. `summarize_sonic_unleashed_preview_select_stage.ps1` walks nested `Category` / `Stage` entries, counts stage `Type` values, records root route categories, and maps route families to the current source-recovery targets: `TitleMenuController`, `SonicDayHudController`, `WerehogHudController`, `WorldMapController`, `ResultScreenController`, `AudioCueCatalog`, and `SequenceRouteController`. Loading and pause are deliberately reported as `not-present-in-select-xml` because they are runtime/package lanes, not direct stage-select routes. Evidence-backed today: route/menu taxonomy, stage/hub/debug route names, archive/appendArchive/IsEvil hints, and prototype-only secondary oracle mapping against our frontend/HUD controller backlog. Still pending: any prototype-specific recomp lane, any direct retail address transfer, exact execution validation of prototype routes, and any claim that prototype behavior is 1:1 retail behavior. The Reddog debug/profiler assets remain the strongest local-only style reference for making SWARD's F2 panel look native, but the DDS payloads stay unpublished.
  - Phase 204 uses that prototype route taxonomy as a starter UI/UX coverage matrix instead of a loose route dump. `summarize_sonic_unleashed_preview_select_stage.ps1` now emits `starterScreenCoverageMatrix` rows for `title-menu`, `loading`, `options-settings`, `pause`, `sonic-day-hud`, `werehog-hud`, `world-map`, `results`, and `audio-sfx`, each with controller name, route evidence status, route count, source-recovery focus, next evidence beat, and the repeated boundary that retail runtime evidence remains primary. Evidence-backed today: title/old-main-menu/world-map/Sonic day/Werehog/results/audio route coverage can be used to prioritize controller recovery; loading and pause remain package/runtime lanes rather than direct `Select.xml` routes. The F2 operator panel also gets a Reddog-style F2 panel `Coverage` tab backed by `kStarterUiCoverageRows` and repo-safe style helpers, using local-only Reddog assets as visual references but not committed payloads.
  - Phase 205 turns the Phase 204 coverage matrix into an explicit source-recovery lane decision: coverage matrix selected Sonic Day HUD retail runtime. The preview route taxonomy script now emits `nextSourceRecoveryLane` with `laneId=sonic-day-hud-retail-runtime`, `decision=continue-retail-runtime-hud-value-recovery`, and `status=coverage-matrix-selected-retail-sonic-day-hud`, plus a `sourceRecoveryLaneQueue` for `world-map-prototype-route`, `results-prototype-route`, and `audio-sfx-retail-runtime`. This is not a new runtime proof; it is the prioritization layer that keeps Sonic Day HUD first because boost/ring-energy normalization, `SetPatternIndex` / `SetHideFlag` gauge state joins, and exact SFX/audio IDs are still the blocking facts for native 1:1 HUD source. The F2 `Coverage` tab now renders the same decision through `DrawOperatorNextSourceRecoveryLane`. The world-map/results prototype lanes queued for later retail runtime proof are useful, but they still need retail runtime owner/capture proof before they become 1:1 source lanes.
