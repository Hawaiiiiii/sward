...
- It now has Asset View plus atlas-sheet gallery navigation, but this is not yet a full DDS/subimage browser.
...
- It now emits authored CSD cast transform, keyframe curve, keyframe sample, sampled transform, sampled draw-command, sampled channel-state, and sampled channel-evaluation descriptors for a first exact-family sample set and draws those sampled transform markers in the preview, but it still does not drive full CSD animation playback.
