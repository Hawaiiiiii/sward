...
| Real-runtime UnleashedRecomp UI Lab parity lane | Early-game alpha is now the default focus: title loop, title menu, title options, loading, and normal Sonic HUD; title/menu/loading direct-context routing is evidence-backed; normal Sonic HUD binds the real `ui_playscreen` CSD project; `ui_prov_playscreen` remains split into a later Extra/Tornado-family target | `W:\b\ui_lab_runtime\UnleashedRecomp\UnleashedRecomp.exe`; default capture set `early-game` |
...
- `100%` complete against the tracked research plan through Phase `104`
