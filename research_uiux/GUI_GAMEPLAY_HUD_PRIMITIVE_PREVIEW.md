...
Phase 60 widens the scene-primitive renderer into the gameplay HUD proxy family. Phase 61 audits the scene ownership labels against parsed `ui_prov_playscreen` deep-analysis facts, and Phase 62 adds primitive animation-bank/frame-cursor cues:
...
- added `--layout-primitive-playback-smoke` to verify recovered animation-bank labels and sampled frame cursors
- updated GUI regression coverage to target `b/rr62/sward_ui_runtime_debug_gui.exe`
...
The GUI was also launch/capture-checked through `PrintWindow` by selecting `Gameplay HUD Hosts -> SonicMainDisplay.cpp`, pressing `Run Host`, pausing playback during `Intro`, and capturing the rr62 window with gameplay HUD primitives visible over the proxy atlas. Phase 61 keeps that visual path and tightens the primitive ownership smoke; Phase 62 adds visible animation-bank/frame-cursor cues.
