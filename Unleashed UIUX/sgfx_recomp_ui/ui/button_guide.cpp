// DECOUPLED from UnleashedRecomp/ui/button_guide.cpp — game runtime (imgui_snapshot /
// gpu/video / hid / aspect patches / config / app / decompressor / embedded DDS) ->
// the shim + render contract. ImFontAtlasSnapshot::GetFont, Config::ControllerIcons,
// hid::g_inputDeviceController and the aspect offsets keep their recomp names (the shim
// provides them); the 2 icon sheets load via render::LoadUISprite. Body byte-identical.
...
std::unique_ptr<sgfx::render::Texture> g_upControllerIcons;
std::unique_ptr<sgfx::render::Texture> g_upKBMIcons;
...
std::tuple<std::tuple<ImVec2, ImVec2>, sgfx::render::Texture*> GetButtonIcon(EButtonIcon icon)
{
    std::tuple<ImVec2, ImVec2> btn;
    sgfx::render::Texture* texture;
...
void ButtonGuide::Init()
{
    ...
    g_upControllerIcons = sgfx::render::LoadUISprite(sgfx::render::UISprite::Controller);
    g_upKBMIcons = sgfx::render::LoadUISprite(sgfx::render::UISprite::KBM);
}
