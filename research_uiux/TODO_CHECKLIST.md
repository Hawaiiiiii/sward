...
- [x] Phase 211 on `2026-05-03`: promoted the Phase 210 local CT evidence into retail runtime hooks for the HUD-relevant Day gameplay writers. `CHudSonicStage_patches.cpp` now wraps the CT-anchored rings writer `sub_82519FE8`, Day boost writer `sub_82A50838`, and lives/status writers `sub_82BDBA20` / `sub_82BDBA60`, emitting `sonic-hud-ct-gameplay-writer` events through `OnSonicHudCtGameplayWriter` with value name, callsite, storage address, previous/current raw value, delta, and float payload for boost. The HUD summarizer now reports `ct_gameplay_writer_groups=`, joins same-frame CT writer evidence against unresolved owner/setter candidate rows as `ct_gameplay_writer_owner_setter_candidate_correlation_groups=`, adds a `ct-gameplay-writer` proof-matrix lane, and keeps `sonic_hud_ct_gameplay_writer_status=ct-anchored-gameplay-writer-evidence-present-pending-final-hud-formula` so this ct-anchored gameplay writer lane drives source recovery without pretending the final boost/ring-energy UI formula or audio IDs are solved.
