...
rem Phase 368 SGFX shell launch profile.
rem
...
rem
rem Always emits the bridge events:
rem   Text:OverridesLoaded:<count>
rem   Text:HostOverrideHit:<key>            (host Localise hits)
rem   Text:CsdOverrideHit:<original>        (guest CSD SetText hits)
rem   Asset:VisibleOverrideHit:<guestPath>  (loose-file resolved)
rem   Asset:OverrideHit:<assetName>         (archive-entry resolved)
rem   Stage:GameplaySkip                    (gameplay-skip armed)
rem   Input:UiOnlyLock                      (UI-only pad mask applied)
...
echo === SGFX shell launch profile (Phase 368) ===
...
pushd "%INSTALL_DIR%"
"%INSTALL_DIR%\UnleashedRecomp.exe" !EXE_ARGS!
set "RC=%errorlevel%"
popd
