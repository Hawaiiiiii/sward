...
Phase 51 added the first native, windowed SWARD UI runtime workbench. Phase 52 added the first visual preview surface, Phase 53 added gameplay-HUD proxy atlas binding, Phase 54 added timer-driven playback controls, Phase 55 added state-aware preview motion, Phase 56 added exact-family preview layouts, Phase 57 added decoded layout-evidence overlays, Phase 58 added frame-domain layout timeline readouts, Phase 59 added the first scene-primitive draw pass, Phase 60 widened primitives into gameplay HUD proxy previews, Phase 61 audits gameplay HUD primitive scene ownership, Phase 62 adds primitive animation-bank/frame-cursor cues, Phase 63 surfaces those cues in the GUI detail pane, Phase 64 adds primitive channel-classification cues, Phase 65 adds a compact primitive channel-count legend in the preview, Phase 66 adds host-level visual parity summaries, Phase 67 adds host-list readiness badges, Phase 68 adds next-renderer blocker cues, Phase 69 adds first exact-family primitive channel sample cues, Phase 70 adds first exact-family draw command descriptors, Phase 71 adds first authored CSD cast transform descriptors, Phase 72 adds first authored CSD keyframe curve descriptors, Phase 73 adds first authored CSD keyframe sample descriptors, Phase 74 adds first authored sampled transform descriptors, Phase 75 draws the first authored sampled transform markers in the preview, Phase 76 adds the first authored sampled draw-command bridge, Phase 77 adds the first sampled non-position channel command, Phase 78 adds reusable sampled channel-state evaluation, Phase 79 adds the first unobstructed asset-viewer mode, Phase 80 hardens asset-root discovery plus gallery navigation, Phase 81 binds Asset View atlas candidates to recovered CSD package/scene/cast/subimage evidence, Phase 82 adds CSD element navigation, Phase 83 adds selected-element crop previews, Phase 84 adds selected CSD cast/subimage draw descriptors, Phase 85 adds selected CSD source/destination draw commands, Phase 86 adds selected CSD render-target plan previews, and Phase 87 adds decoded local DDS blit previews:
...
- exposes an `Asset View` / `Runtime View` toggle for unobstructed local atlas inspection
- exposes `Asset Prev` / `Asset Next` controls for sorted local atlas-sheet navigation
- exposes `Element Prev` / `Element Next` controls for walking the selected host's CSD element bindings
- resolves local atlas sheets from either repo-root or build-dir launches, with optional `SWARD_UI_ASSET_ROOT` override
...
- adds `Layout primitive draw commands:` detail-pane descriptors in the form `scene:x,y,widthxheight:channels@frame/count`
- adds `Authored cast transforms:` detail-pane descriptors from parsed exact-family CSD cast data
- adds `Authored keyframe curves:` detail-pane descriptors from parsed exact-family CSD animation data
- adds `Authored keyframe samples:` detail-pane descriptors with deterministic sampled values from parsed exact-family CSD keyframes
- adds `Authored sampled transforms:` detail-pane descriptors that combine authored cast rectangles with sampled X/Y keyframe values
- draws authored sampled transform markers for the first Title and Loading exact-family samples in the preview panel
- adds `Authored sampled draw commands:` detail-pane descriptors and draws authored sampled markers from command objects
- carries the first authored sampled `Color` channel state for Pause draw commands and derives channel alpha diagnostics
- evaluates authored sampled draw-command state into alpha, visibility, and cast-space deltas for the first Title/Pause/Loading command set
...
- draws selected host atlas sheets in Asset View with runtime/debug overlays suppressed and asset aspect ratio preserved
- draws navigable CSD element-binding cues in Asset View for Title, Pause, Loading, and gameplay-HUD proxy families
- reports `CSD element bindings:` in the detail pane, including package, scene, cast, scene-cast count, scene-subimage count, and package-subimage count
- reports `CSD element selection:` in the detail pane and highlights only the selected element label while keeping neighboring element markers visible
- reports `CSD element crop:` in the detail pane and draws a selected-element crop inset in Asset View
- reports `CSD cast/subimage:` in the detail pane and draws a selected cast/subimage descriptor cue in Asset View
- reports `CSD subimage draw command:` in the detail pane and draws a selected source/destination draw-command cue in Asset View
- reports `CSD render plan:` in the detail pane and draws a mini `1280x720` render-target preview in Asset View
- reports `CSD DDS blit:` in the detail pane and draws decoded local DXT5 source rects into that mini render-target preview when source textures are available
...
- supports `--asset-view-smoke` so automation can verify the current local atlas viewer inventory without opening a window
- supports `--asset-gallery-smoke` so automation can verify sorted atlas-sheet navigation without opening a window
- supports `--asset-csd-binding-smoke` so automation can verify CSD element bindings without opening a window
- supports `--asset-csd-navigation-smoke` so automation can verify multi-element CSD navigation without opening a window
- supports `--asset-csd-crop-smoke` so automation can verify selected-element crop/footprint rectangles without opening a window
- supports `--asset-csd-subimage-smoke` so automation can verify selected cast/subimage descriptors without opening a window
- supports `--asset-csd-draw-command-smoke` so automation can verify selected source/destination draw commands without opening a window
- supports `--asset-csd-render-plan-smoke` so automation can verify selected CSD render-target rectangles without opening a window
- supports `--asset-csd-dds-blit-smoke` so automation can verify local DDS source decode and selected source/destination blit rects without opening a window
...
- supports `--layout-draw-command-smoke` so automation can verify exact-family primitive draw command descriptors without opening a window
- supports `--authored-cast-transform-smoke` so automation can verify exact-family authored CSD cast transform descriptors without opening a window
- supports `--authored-keyframe-curve-smoke` so automation can verify exact-family authored CSD keyframe curve descriptors without opening a window
- supports `--authored-keyframe-sample-smoke` so automation can verify exact-family authored CSD keyframe sample descriptors without opening a window
- supports `--authored-sampled-transform-smoke` so automation can verify exact-family authored sampled transform descriptors without opening a window
- supports `--authored-sampled-transform-preview-smoke` so automation can verify preview marker geometry without opening a window
- supports `--authored-sampled-draw-command-smoke` so automation can verify renderer-facing sampled draw commands without opening a window
- supports `--authored-sampled-channel-command-smoke` so automation can verify sampled non-position channel state without opening a window
- supports `--authored-sampled-channel-eval-smoke` so automation can verify sampled channel alpha, visibility, and cast-space deltas without opening a window
...
sward_ui_runtime_debug_gui asset csd subimage smoke ok descriptors=9 sonic=so_speed_gauge/position_hd->Cast_0506_bg:sub=1:tex=ui_ps1_gauge1.dds:uv=0.015625,0.500000-0.078125,0.656250:cast=16x20:pos=0.093750,0.009722:scale=1.00,1.00 loading=bg_1/arrow->img_1:sub=198:tex=mat_load_comon_001.dds:uv=0.464844,0.168056-0.699219,0.501389:cast=300x240:pos=-0.109375,0.166667:scale=1.00,1.00 title=mm_bg_usual/black3->black3:sub=14:tex=ui_mm_parts1.dds:uv=0.700000,0.525000-0.712500,0.550000:cast=16x16:pos=0.155469,0.426389:scale=23.00,29.00 pause=bg/img->img:sub=none:tex=none:cast=1280x720:pos=0.000000,0.000000:scale=1.00,1.00
sward_ui_runtime_debug_gui asset csd draw command smoke ok sonic=so_speed_gauge/position_hd->Cast_0506_bg:tex=ui_ps1_gauge1.dds:src=4,64,16x20:dst=16x20:pos=0.093750,0.009722 loading=bg_1/arrow->img_1:tex=mat_load_comon_001.dds:src=595,121,300x240:dst=300x240:pos=-0.109375,0.166667 title=mm_bg_usual/black3->black3:tex=ui_mm_parts1.dds:src=896,336,16x16:dst=368x464:pos=0.155469,0.426389 pause=bg/img->img:fill=cast:dst=1280x720:pos=0.000000,0.000000
sward_ui_runtime_debug_gui asset csd render plan smoke ok sonic=so_speed_gauge/position_hd->Cast_0506_bg:texture:src=4,64,16x20:dst=752,357,16x20 loading=bg_1/arrow->img_1:texture:src=595,121,300x240:dst=350,360,300x240 title=mm_bg_usual/black3->black3:texture:src=896,336,16x16:dst=655,435,368x464 pause=bg/img->img:fill:dst=0,0,1280x720
sward_ui_runtime_debug_gui asset csd dds blit smoke ok sonic=ui_ps1_gauge1.dds:DXT5:256x128:src=4,64,16x20:dst=752,357,16x20 loading=mat_load_comon_001.dds:DXT5:1280x720:src=595,121,300x240:dst=350,360,300x240 title=ui_mm_parts1.dds:DXT5:1280x640:src=896,336,16x16:dst=655,435,368x464 pause=fill:dst=0,0,1280x720
...
- It now has Asset View plus atlas-sheet gallery navigation, CSD element navigation, selected-element crop previews, selected cast/subimage descriptors, source/destination draw-command descriptors, mini render-target plan previews, and first decoded DDS source blits, but this is not yet a full DDS/subimage browser.
...
- It now emits exact-family primitive draw command descriptors, but the descriptors still use recovered normalized geometry rather than decoded authored node transforms.
- It now emits authored CSD cast transform, keyframe curve, keyframe sample, sampled transform, sampled draw-command, sampled channel-state, sampled channel-evaluation descriptors, navigable Asset View CSD element bindings, selected-element crop descriptors, selected cast/subimage draw descriptors, selected source/destination draw-command descriptors, selected render-target plans, and first decoded DDS source blits for a curated exact/proxy sample set, but it still does not drive full CSD animation playback.
...
The first non-CLI executable is present, Phase 52 added the first visual preview panel, Phase 53 made the gameplay-HUD host preview useful enough to inspect in the window, Phase 54 keeps intro/action bands visible through timer-driven playback, Phase 55 gives those bands visible preview motion, Phase 56 splits exact Title/Pause/Loading placement away from the generic role stack, Phase 57 puts parsed layout/timeline evidence into the same visual surface, Phase 58 adds frame-domain progress over those recovered timelines, Phase 59 starts drawing recovered scene primitives, Phase 60 widens that primitive layer to gameplay HUD proxy previews, Phase 61 guards the gameplay HUD proxy primitive ownership against the parsed deep-analysis scene facts, Phase 62 attaches recovered animation-bank names plus sampled frame cursors to that primitive layer, Phase 63 makes the same evidence readable in the operator detail pane, Phase 64 classifies the recovered track summaries into typed channel cues, Phase 65 adds a compact preview legend for those channel counts, Phase 66 folds the atlas/layout/primitive/channel readiness into a single detail-pane parity summary, Phase 67 pushes that readiness signal into the host browser, Phase 68 adds the next-renderer blocker directly to the parity readout, Phase 69 gives exact-family primitives a stable channel-sample token surface, Phase 70 turns that sampled primitive evidence into deterministic draw command descriptors, Phase 71 binds the first parsed authored CSD cast transforms into the same GUI path, Phase 72 binds the first parsed authored CSD keyframe curves into the detail path, Phase 73 adds deterministic authored keyframe sample values, Phase 74 combines sampled transform values with authored cast rectangles, Phase 75 draws those sampled authored transform markers in the preview, Phase 76 wraps those markers in renderer-facing sampled draw commands, Phase 77 starts carrying sampled non-position channel state, Phase 78 evaluates sampled alpha, visibility, and cast-space deltas from those commands, Phase 79 adds an unobstructed asset-viewer mode, Phase 80 makes that asset-viewer path launch-location-safe while adding sorted sheet navigation, Phase 81 starts binding asset sheets to individual recovered CSD elements, Phase 82 makes those element bindings navigable in the window, Phase 83 draws the first selected-element crop preview inset, Phase 84 adds selected cast/subimage draw descriptors from parsed CSD evidence, Phase 85 derives source/destination draw-command descriptors from those selections, Phase 86 projects those commands into target-space render plans, and Phase 87 decodes the first local DDS source textures into that blit path.
...
2. Widen decoded DDS blits from selected samples into a multi-cast family-specific render pass.
3. Decode more layout node/timeline data into draw commands rather than atlas-sheet previews.
4. Add CSD-style timeline playback for intro/idle/outro bands.
5. Replace more local-only scaffold ownership with translated PPC-backed host behavior.

Full 1:1-style UI/UX parity for all relevant Sonic Unleashed UI families is still a long-range target, not a single remaining build step. The honest state is: the proper `.exe` shell exists now; visual timing/motion scaffolding exists; first exact-family placement adapters exist; parsed layout evidence, frame-domain timeline readouts, diagnostic scene primitives, audited gameplay HUD proxy primitives, primitive animation/frame cues, readable primitive detail summaries, primitive channel cues, primitive channel legends, host-level visual parity summaries, host-list readiness badges, next-renderer blocker cues, first exact-family primitive channel sample tokens, first exact-family draw command descriptors, first authored CSD cast transform descriptors, first authored CSD keyframe curve descriptors, first authored CSD keyframe sample descriptors, first authored sampled transform descriptors, first authored sampled transform preview markers, first authored sampled draw-command descriptors, first authored sampled channel-state descriptors, first authored sampled channel-evaluation descriptors, the first unobstructed asset-viewer mode, cwd-safe atlas gallery navigation, navigable CSD element cues, selected-element crop previews, selected cast/subimage descriptor cues, selected source/destination draw-command cues, selected render-target plan previews, and first decoded local DDS source blits are visible in the window; the full asset/layout/animation renderer is the next major productization track.
