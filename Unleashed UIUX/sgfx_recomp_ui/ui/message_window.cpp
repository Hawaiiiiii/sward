// DECOUPLED from UnleashedRecomp/ui/message_window.cpp — the game runtime (api/SWA.h,
// gpu/imgui/imgui_snapshot, gpu/video, hid/hid, locale/locale, app, decompressor,
// exports, sdl_listener) -> the shim + render contract + the sibling UI helpers
// (imgui_utils / button_guide). ImFontAtlasSnapshot::GetFont keeps its recomp name (the
// shim provides it). The menu BODY is byte-identical to the recomp; the only HARD game
// couplings left as-is (and reported for the orchestrator's shim) are the SDL event
// listener, App::s_isInit, the hid:: input-state helpers, Game_PlaySound, the JP-locale
// check (Config::Language/ELanguage) and the in-game SWA::CInputState pad read.
#include "message_window.h"
#include "button_guide.h"
#include "imgui_utils.h"
#include "../platform/sgfx_platform.h"
#include <imgui_internal.h>     // ImGui internals the recomp pulled in transitively
#include <algorithm>            // std::clamp / std::max
#include <array>                // std::array (button-guide pairs)
