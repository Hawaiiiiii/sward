// DECOUPLED from UnleashedRecomp/ui/game_window.cpp. This is the SDL/OS WINDOW module, not an
// ImGui menu, so SDL/SDL_syswm and the Win32 DWM/DPI headers stay as legitimate platform deps.
// REMOVED game-runtime includes: <gpu/video.h> (g_needsResize), <os/logger.h> (LOG* macros ->
// os::logger::Log), <os/user.h> (os::user::IsDarkTheme), <os/version.h> (os::version::GetOSVersion),
// <app.h> (App::s_isSaving / App::Exit / App::s_language), <sdl_listener.h> (GetEventListeners),
// <res/images/game_icon*.bmp.h> (embedded icon BMP byte arrays). All of these keep their EXACT
// recomp names below — the orchestrator binds them via the shim (reported in newShimSymbols /
// gameStateCoupling). The sdl_events.h helpers (SDL_ResizeEvent/SDL_MoveEvent/SDL_USER_EVILSONIC)
// likewise stay by name. Body byte-identical to the recomp.
...
// std headers the removed game/OS headers pulled in transitively.
#include <cstddef>
#include <cstdint>
#include <limits>
#include <set>
#include <utility>
#include <vector>
#include <fmt/format.h>   // GetTitle()'s resize-title fmt::format (was via os/logger.h's stdafx)
