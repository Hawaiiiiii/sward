...
- Phase 184 established that the native Recomp Profiler is the primary operator surface for UI Lab sessions by embedding compact SWARD UI Lab readouts, HUD value binding status, capture state, and the key HUD SGlobals directly into the existing `DrawProfiler()` panel. Phase 185 supersedes that attachment for day-to-day play: F2 toggles the detached SWARD UI Lab panel, F1 remains reserved for the native Recomp Profiler, the older counter/view/export/debug-draw windows remain opt-in Legacy floating panes, and the live bridge/API plus debug-menu-fork typed fields stay enabled by default.
