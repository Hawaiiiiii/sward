...
struct Options
{
    ...
    bool readback = false;
    bool readbackLoop = false;
    uint32_t readbackEvery = 6u;
...
Options parseOptions(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (arg == "--readback-loop")
            { options.readback = true; options.readbackLoop = true; }
        else if (arg == "--readback-every")
            options.readbackEvery = parseUint(requireValue("--readback-every"), "--readback-every");
...
{
public:
    SdlWindow(const char* title, uint32_t width, uint32_t height, bool hidden = false)
        : m_window(SDL_CreateWindow(title, static_cast<int>(width), static_cast<int>(height),
...
public:
    ...
    void resetReadback() { m_pixelsRead = false; m_pixelsReadOk = false; }
                               ...
                               ramses::ERendererEventResult result) override
    {
        ...
        {
            const std::string tmp = m_screenshotPath + ".tmp";
            m_screenshotSaved = ramses::RamsesUtils::SaveImageBufferToPng(tmp, pixels, m_width, m_height, true);
            if (m_screenshotSaved)   // atomic publish so a reader never sees a partial PNG
            {
                std::error_code rc;
                std::filesystem::rename(tmp, m_screenshotPath, rc);
                m_screenshotSaved = !rc;
            }
        }
...
int main(int argc, char** argv)
{
    try
    {
        ...
        SdlWindow window("SGFX Cine Real Ramses Scene", displayDimensions.width, displayDimensions.height, options.readbackLoop);
        ...
        // The live loop renders to an offscreen buffer so the readback is independent of
        // the window (it can stay hidden); the one-shot snapshot keeps the framebuffer.
        ramses::displayBufferId_t renderTarget = displayFramebuffer;
        if (options.readbackLoop)
        {
            renderTarget = renderer.createOffscreenBuffer(display, displayDimensions.width, displayDimensions.height);
            renderer.setDisplayBufferClearColor(display, renderTarget, ramses::vec4f{0.02f, 0.03f, 0.05f, 1.0f});
            renderer.flush();
            for (int i = 0; i < 10; ++i)
                renderer.doOneLoop();
            SDL_HideWindow(window.get());   // Ramses shows the window on createDisplay; the offscreen
                                            // buffer renders without it, so hide it for in-pane use
        }
        ...
        if (!sceneControl.setSceneDisplayBufferAssignment(sceneId, renderTarget, 0))
        ...
        while (renderedFrames < options.frames)
        {
            ...
            if (options.readbackLoop && options.readbackEvery > 0u && (renderedFrames % options.readbackEvery == 0u))
            {
                handler.resetReadback();
                renderer.readPixels(display, renderTarget, 0u, 0u, displayDimensions.width, displayDimensions.height);
                renderer.flush();
                pumpUntil(renderer, sceneControl, handler, [&] { return handler.pixelsRead(); }, [&] {
                    updateLogicAndViewControl(renderedFrames, options.orbit, "loop readback");
                    scene->flush();
                }, std::chrono::seconds(2));   // tolerant: the callback saves atomically; never throws here
            }
        }

        if (options.readback && !options.readbackLoop)
        {
            renderer.readPixels(display, renderTarget, 0u, 0u, displayDimensions.width, displayDimensions.height);
            renderer.flush();
