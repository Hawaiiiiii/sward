...
- Phase 233 merges the profiler and SWARD UI Lab back into one native in-game workspace. The right rail defaults visible while UI Lab is enabled, keeps the native frame-time graph and compact profiler summary above the SWARD tabs, mirrors the runtime/evidence stream through the in-game Console tab, and leaves F2 unused so the lab no longer competes with the game window sizing behavior. F1 hides/shows the merged panel, the first placement is fixed to the right side but the operator can move/resize it afterward, and the long native profiler readout stays available through the `Full profiler details` option.
- Phase 246 adds live bridge controls for the native foreground render probe. `native-foreground-attach` / `native-foreground-detach` arm or clear the selected native CSD scene without replacing game-owned pointers, while `native-motion-play` / `native-motion-stop` / `native-motion-scrub <frame>` drive the resolved scene's real `m_MotionFrame`. `native-foreground-status` reports the Make probe, native scene pointer, render-pass sighting, render count, and `ownerHijackUsed=false` so the next gate is render-count/visual proof before any foreground owner attach experiment.
- Phase 247 lets tools queue the same Native CSD Make/Observe probe without touching the in-game panel: `native-make-observe <project> <scene> [frame]` resolves a YNCP/XNCP project by name/path, resolves the scene row, and waits for the game to create that CSD naturally before traversal captures the native tree. This keeps automation on the non-crashing observe path by default.
- Phase 250 adds a safety gate to the Native Foreground Render Probe after cross-project piggybacking proved unsafe: the selected native scene only renders during an active `CScene::Render` pass from the same captured CSD project. Mismatched or unknown host passes are reported as blocked instead of calling into the native renderer, so arbitrary foreground spawning remains an owner/host attach problem rather than a blind render-call problem.
- Phase 251 splits the native CSD result into the resource `Scene*` seen during project traversal and the live manager `CScene*` seen during `CScene::Render`. The UI Lab now correlates those pointers before allowing motion writes, which keeps the next experiments on the real renderable scene object instead of the parsed resource tree. A live crash proved that extra `CScene::Render` calls are still unsafe without foreground ownership, so Attach Native Scene now acts as a same-project manager compatibility probe until the real owner/host attach path is resolved.
- Phase 188 keeps that boundary honest by adding semantic `ui_playscreen/...` path candidates for correlated HUD node writes. The bridge can now say that an unresolved speed/tutorial/boost-ring-energy node is a candidate for stable paths like `ui_playscreen/add/speed_count/position/num_speed`, `ui_playscreen/add/u_info`, `ui_playscreen/so_speed_gauge`, or `ui_playscreen/so_ringenagy_gauge`, while exact path resolution remains reserved for owner fields, child lookup chains, or draw-list matches.
...
- `local_build_env\ur103clean\b\ui_lab_runtime\UnleashedRecomp\UnleashedRecomp.exe` builds successfully from the generated clone after the local SDL Clang 22 shim patch and tracked `runtimeobject` link fix. The build helper may create a transient `W:` SUBST alias while compiling to avoid Windows path-length/space issues, but it is no longer the persistent runtime path.
...
- Manual SWARD UI Lab launch must use the real complete install root plus `--use-cwd`. `research_uiux/runtime_reference/tools/launch_unleashed_recomp_ui_lab_manual.ps1` now copies the patched build and matching D3D12/DXC DLLs into an ignored sidecar under `Unleashed Recomp - Windows (Complete Installation) 1.0.3`, then starts that sidecar from the complete install working directory so it does not fall into the installer/config path.
