...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    static get observedAttributes() {
      return ['shape', 'radius', 'mask', 'fit', 'position', 'placeholder', 'src', 'id', 'uv'];
    }
    ...
    // Parse the optional `uv` attribute -> normalised [u0,v0,u1,v1] sub-rect of
    // the source sheet, or null. Used to crop a single sprite out of a texture
    // atlas (the game packs many UI sprites per sheet and addresses them by UV).
    _uv() {
    ...
    _applyView() {
      // Atlas UV crop: stretch sub-rect [u0,v0]-[u1,v1] of the sheet to fill the
      // frame, top-left anchored (overriding the stylesheet's centring). Applies
      // only to the author `src` atlas — a user's own dropped image (this._userUrl)
      // replaces the whole slot and is framed normally.
      const uv = this._userUrl ? null : this._uv();
      if (uv) {
        ...
        this._img.style.width = (100 / du) + '%';
        this._img.style.height = (100 / dv) + '%';
        this._img.style.left = (-uv[0] / du * 100) + '%';
        this._img.style.top = (-uv[1] / dv * 100) + '%';
        return;
      }
