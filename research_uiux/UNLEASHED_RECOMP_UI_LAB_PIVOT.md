...
- Default-open operator windows for the useful debug-menu-fork workflow: welcome/status, inspector, counters, view, exports, debug draw, Stage/HUD operator, and live API panes now appear immediately in visible UI Lab sessions. The hidden-overlay path remains clean for native capture.
...
- A visible operator-shell smoke under `out/ui_lab_runtime_evidence/20260427_194221/` passed on the current build with default-open operator windows and wrote `ui_lab_live_state.json` beside the screenshot evidence.
