...
- [x] Phase 182 on `2026-04-30`: added a repo-safe manual Sonic HUD value observer summarizer for `ui_lab_events.jsonl`, reporting CSD text writes, gauge scale/pattern/hide writes, late-resolved node writes, gameplay-value updates, callsite classifications, proven `ui_playscreen` paths, values, and sources so manual gameplay sessions can be evaluated immediately after the user drives into real HUD gameplay.
- [x] Phase 183 on `2026-04-30`: extended the manual Sonic HUD observer summarizer with a manual unresolved node resolver that groups `sonic-hud-node-write-unresolved` events by guest node address, counts write kinds/values/frame ranges/sources, and labels unresolved Sonic HUD node candidates as numeric text counters or gauge/prompt candidates so the next runtime pass can target exact `ui_playscreen` path resolution instead of raw JSONL scanning.
- [x] Phase 184 on `2026-04-30`: embedded SWARD UI Lab readouts into the native Recomp Profiler via `DrawProfilerAddon()`, made the older floating operator panes opt-in through `Legacy floating panes`, promoted the key HUD SGlobals (`ms_IsRenderHud`, `ms_IsRenderGameMainHud`, `ms_IsRenderHudPause`) into the profiler surface for visual isolation, and added score CSD text path resolution for `ui_playscreen/score_count/score` / `num_score` so resolved score writes become named gameplay HUD values instead of anonymous node writes.
- [x] Phase 185 on `2026-04-30`: split profiler ownership for manual gameplay observation: F1 now stays as the native Recomp Profiler only, F2 toggles a detached SWARD UI Lab profiler-style panel, and SWARD's own panel carries the key HUD render-gate controls with `ms_IsRenderHud` documented as the whole UI/UX gate and `ms_IsRenderGameMainHud` / `ms_IsRenderHudPause` as child HUD/pause render lanes.
- [x] Phase 186 on `2026-04-30`: restored the old embedded-profiler SWARD UI Lab content inside the detached F2 panel as its own `SWARD UI Lab` tab, kept F1 as native Recomp Profiler only, and added `hudRenderGateCorrelation` plus `sonic-hud-render-gate-correlated` evidence so `ms_IsRenderHud` / `ms_IsRenderGameMainHud` / `ms_IsRenderHudPause` can be correlated with unresolved `ui_playscreen` node writes and known caller groups while chasing the remaining Sonic HUD value paths.
- [x] Phase 187 on `2026-04-30`: restyled the detached F2 SWARD panel with the native Profiler font and ImPlot frame-time plot, kept the old SWARD UI Lab tab as the focused operator surface, and added `sonic-hud-node-write-callsite-correlated` evidence so unresolved Sonic HUD node writes can be labeled as `timer/speed/boost-ring-energy/tutorial` candidates from same-frame or nearby generated-PPC HUD callsite samples.
- [x] Phase 188 on `2026-04-30`: promoted correlated unresolved Sonic HUD node writes into semantic `ui_playscreen/...` path candidates with `sonic-hud-node-write-semantic-path-candidate`, keeping them separate from exact resolved paths while naming likely speed, boost, ring-energy, and tutorial lanes for the next manual gameplay proof.
- [x] Phase 189 on `2026-04-30`: hardened manual SWARD UI Lab launch with a repo-safe helper that always targets `Unleashed Recomp - Windows (Complete Installation) 1.0.3`, stages the patched runtime into an ignored sidecar with matching DXC/D3D12 DLLs, and launches with `--use-cwd` so the real game folder stays active instead of the installer/config flow.
- [x] Phase 190 on `2026-04-30`: upgraded the manual Sonic HUD value summarizer with semantic candidate stability groups, reporting `semantic_candidate_groups` as `ui_playscreen/...:semanticValueName=count` pairs so repeated speed/boost/ring-energy/tutorial candidates can be separated from one-off unresolved node noise before exact child-node paths are solved.
- [x] Phase 191 on `2026-04-30`: bound stable Sonic HUD semantic candidates into typed runtime/controller gameplay values without claiming exact node resolution; the runtime emits `sonic-hud-node-write-semantic-bound` with `semantic-candidate-bound-pending-exact-child-node-resolution`, while `SonicDayHudController::applyRuntimeSemanticPathCandidate` consumes candidate-backed speed, boost, ring-energy, and tutorial lanes as reusable source pending final exact `ui_playscreen/...` child/cast/node resolution.
- [x] Phase 192 on `2026-04-30`: taught the manual Sonic HUD value summarizer to separate semantic-bound runtime-fed evidence from weaker semantic candidates and exact resolved paths, adding `semantic_bound`, `semantic_bound_paths`, and `semantic_bound_groups` so the Phase 191 semantic-bound tutorial lane (`ui_playscreen/add/u_info:tutorialPrompt`) is promoted as stronger runtime evidence while still marked pending exact child-node resolution.
- [x] Phase 193 on `2026-04-30`: folded the Phase 192 manual gameplay evidence back into the reusable Sonic Day HUD controller lane by distinguishing strict semantic-bound inputs (`elapsedFrames`, `speedKmh`, `tutorialPrompt`) from candidate-only boost/ring-energy observations. `SonicDayHudRuntimeSemanticPathCandidateObservation` now carries `bindingStatus`, `--phase193-sonic-hud-semantic-bound-smoke` applies only semantic-bound lanes to the strict controller state, and boost/ring-energy remain explicitly `semantic-candidate-only-pending-runtime-bound` until exact child/cast/node resolution proves them.
- [x] Phase 194 on `2026-04-30`: used live `ui-draw-list` evidence to promote boost/ring-energy gauge draw child paths without overclaiming gameplay setter resolution. The summarizer now accepts `-DrawListPath`, reports `gauge_draw_path_groups`, `gauge_setter_node_candidates`, and `gauge_child_path_status=runtime-draw-list-exact-child-paths;setter-node-address-join-pending`, while controller smoke source records exact visible gauge fill parents `ui_playscreen/so_speed_gauge/position/speed_gauge_color` and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color`; SetScale/SetPatternIndex/SetHideFlag node-to-child joins remain explicitly `setter-node-address-join-pending`.
- [x] Phase 195 on `2026-04-30`: added the setter-node address join needed to promote boost/ring-energy from visible gauge geometry toward runtime-fed HUD values. The summarizer now emits `gauge_setter_child_path_joins` and upgrades status to `runtime-draw-list-setter-node-joined` only when an unresolved SetScale/SetPatternIndex/SetHideFlag node matches a live draw-list `castNodeAddress` or `layerAddress` under the semantic gauge path; controller source adds `SonicDayHudRuntimeGaugeSetterChildPathJoin` / `applyRuntimeGaugeSetterChildPathJoin`, proving `ui_playscreen/so_speed_gauge/position/speed_gauge_color/Cast_0506` and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color/Cast_0483` as exact child-path inputs with `setter-node-address-join-runtime-proven` while SFX/audio IDs remain pending.
- [x] Phase 196 on `2026-04-30`: followed the Phase 195 manual gameplay evidence into the `sub_824D6C18` rolling counter lane instead of overclaiming a missing gauge setter join. The summarizer now reports `rolling_counter_semantic_groups` and `boost_ring_energy_status=rolling-counter-text-candidate-pending-gauge-state-normalization` for `kind=text` boost/ring-energy candidates from `sub_824D6C18`, while controller source adds `SonicDayHudRuntimeRollingGaugeCounterObservation` / `applyRuntimeRollingGaugeCounterObservation` so reusable HUD source can carry the candidate evidence without mutating final boost/ring-energy gauge values until exact gauge-state normalization or setter-child resolution proves it; this is pending gauge-state normalization, not a final 1:1 boost/energy value.
- [x] Phase 197 on `2026-05-01`: traced the `sub_824D6C18` rolling counter one level deeper into a parallel owner-field rolling counter lane without overclaiming exact offset semantics. A new `sonic-hud-owner-gauge-snapshot` runtime event in the `sub_824D6C18` PPC hook captures `CHudSonicStage` owner-field dword reads at `+460/+464/+468/+472/+480` under the existing 600-frame stable-signature throttle, the summarizer adds `owner_field_rolling_counter_groups=owner=...:field+N=valueName:samples=M` and `owner_field_rolling_counter_status=owner-field-rolling-counter-pending-exact-offset-normalization`, and reusable controller source adds `SonicDayHudRuntimeOwnerFieldRollingCounterObservation` / `applyRuntimeOwnerFieldRollingCounterObservation` plus `formatSonicDayHudRuntimeBindingPhase197SmokeSequence` (`--phase197-sonic-hud-owner-field-rolling-counter-smoke`); boost and ring-energy gauge final values explicitly remain at controller defaults, per-offset semantics remain pending exact normalization, and `gauge_setter_child_path_joins` remains empty unless later runtime evidence proves it.
- [x] Phase 198 on `2026-05-01`: opened the owner-field-to-setter-scale correlation lane to disambiguate which offset is the integer rolling counter vs. the float fill scale vs. the hide/style enum vs. the animation phase, by joining each Phase 197 owner-field snapshot with the same-frame `CSD::CNode::SetScale` write on the proven gauge-fill cast nodes `Cast_0506` (boost) and `Cast_0483` (ring-energy). The runtime caches the latest `OwnerFieldGaugeSnapshotCache` after each `OnHudSonicStageOwnerFieldGaugeSnapshot` call, then `OnCsdNodeSetScale` emits a new `sonic-hud-gauge-scale-owner-correlated` evidence event whenever a same-frame SetScale on `ui_playscreen/so_speed_gauge/...` or `ui_playscreen/so_ringenagy_gauge/...` lines up with a populated cache; the summarizer adds `owner_field_gauge_scale_correlation_groups=path=...:owner=...:field+N:joins=M` and `owner_field_gauge_scale_correlation_status=owner-field-gauge-scale-correlation-pending-formula-proof`; reusable controller source adds `SonicDayHudRuntimeOwnerFieldGaugeScaleCorrelation` / `applyRuntimeOwnerFieldGaugeScaleCorrelation` plus `formatSonicDayHudRuntimeBindingPhase198SmokeSequence` (`--phase198-sonic-hud-owner-field-gauge-scale-correlation-smoke`). Boost and ring-energy gauge final values still remain at controller defaults until the per-offset formula is proven; the next runtime beat should prove that formula, retry `gauge_setter_child_path_joins`, and extend the join lane to `CSD::CNode::SetPatternIndex` / `CSD::CNode::SetHideFlag` before any 1:1 value claim.
- [x] Phase 199 on `2026-05-01`: added an honest owner-field offset classification lane that consumes only the Phase 197 snapshot evidence and the Phase 198 same-frame SetScale-join evidence already in `ui_lab_events.jsonl` and tags each `CHudSonicStage` owner-field offset with a *shape* candidate label without claiming a formula. The summarizer aggregates per-offset observed integer values across both gauge `valueName`s of Phase 197 groups and joins from Phase 198 groups, then `Resolve-OwnerFieldOffsetCandidateLabel` classifies the offset by cardinality and observed range into `low-cardinality-narrow-range-candidate` (likely hide flag / style enum), `moderate-cardinality-narrow-range-candidate` (likely animation phase index), `high-cardinality-narrow-range-candidate` (likely fixed-point fill scale), `high-cardinality-wide-range-candidate` (likely rolling counter / wrap accumulator), or `unclassified-pending-more-evidence`. The summarizer reports `owner_field_offset_classifications=owner=...:field+N:cardinality=C:min=Lo:max=Hi:joins=J:candidate=label` and `owner_field_offset_classification_status=owner-field-offset-classification-pending-formula-proof`, kept independent from `boost_ring_energy_status`, `owner_field_rolling_counter_status`, and `owner_field_gauge_scale_correlation_status`. Reusable controller source adds `SonicDayHudRuntimeOwnerFieldOffsetClassification` / `applyRuntimeOwnerFieldOffsetClassification` / `formatSonicDayHudRuntimeBindingPhase199SmokeSequence` (`--phase199-sonic-hud-owner-field-offset-classification-smoke`); boost and ring-energy gauge final values remain at controller defaults because shape labels are not formulas. No runtime hook change was needed in this phase: the classifier is pure post-processing of existing Phase 197/198 evidence channels.
- [x] Phase 200 on `2026-05-01`: tested whether the existing Phase 197/198/199 evidence in `ui_lab_events.jsonl` can prove a formula mapping `CHudSonicStage` owner-field offsets `+460/+464/+468/+472/+480` to the boost / ring-energy gauge fill scales on `Cast_0506` / `Cast_0483`, and added owner-field offset transition diagnostics to surface what the Phase 197 snapshot stream can and cannot say on its own. The latest manual capture stayed on the title loop, so it carries 26 owner-gauge snapshots but zero `sonic-hud-gauge-scale-write` and zero `sonic-hud-gauge-scale-owner-correlated` joins on the gauge fill cast nodes — therefore no per-offset → SetScale formula can be proven across multiple frames from this evidence, and no controller value gets promoted. Boost and ring-energy gauge final values stay at controller defaults; `boost_ring_energy_status`, `owner_field_rolling_counter_status`, `owner_field_gauge_scale_correlation_status`, and `owner_field_offset_classification_status` all remain pending. Concrete summarizer-only diagnostic added: per (`ownerAddress`, `fieldOffset`) the consecutive-pair transition statistics across the snapshot timeline are computed in chronological order and reported as `owner_field_offset_transition_diagnostics=owner=...:field+N:trend=stable|increasing|decreasing|non-monotonic|no-transitions:wrap=true|false:transitions=T:inc=I:dec=D:eq=E:min_delta=M:max_delta=X` plus a parallel `owner_field_offset_transition_diagnostics_status=owner-field-offset-transition-diagnostics-pending-formula-proof`. New PowerShell helpers `New-OwnerFieldOffsetTransitionTrack`, `Add-OwnerFieldOffsetTransitionSample`, `Resolve-OwnerFieldOffsetMonotonicTrendLabel` keep the temporal order that the existing `New-OwnerFieldRollingCounterGroup` SortedSet discards, so wrap detection (any decreasing transition) and shape mix (inc/dec/eq counts) can be read directly from the summary. No runtime hook change, no controller struct change, no fake 1:1 promotion of any gauge value.
- [x] Phase 201 on `2026-05-01`: added an owner-field-to-visible-gauge-path bridge across two already-proven lanes without promoting fake boost/ring-energy values. The Phase 201 manual gameplay run produced high-volume Phase 197 owner-field rolling-counter samples and a live `ui-draw-list` snapshot with exact visible gauge children `ui_playscreen/so_speed_gauge/position/speed_gauge_color/Cast_0506` and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color/Cast_0483`, but still zero `sonic-hud-gauge-scale-write` / `sonic-hud-gauge-scale-owner-correlated` events. The summarizer now reports an owner-field-to-visible-gauge-path bridge as `owner_field_draw_path_bridge_groups=path=...:value=...:owner=...:field+N:samples=S:draws=D` plus `owner_field_draw_path_bridge_status=owner-field-draw-path-bridge-pending-formula-proof`, showing which owner-field candidates are attached to exact visible gauge paths by semantic value while still keeping `boost_ring_energy_status=rolling-counter-text-candidate-pending-gauge-state-normalization`. This is a stronger breadcrumb for native HUD source, not a final formula, not a setter-node join, and not a controller value promotion.
- [x] Phase 202 on `2026-05-02`: added a repo-safe preview build secondary oracle inventory lane with an explicit rule: do not replace the retail runtime oracle. `inventory_sonic_unleashed_preview_build.ps1` records metadata-only facts from a local Sonic Unleashed preview build root: XEX/shader presence, `.ar.00` archive families, UI-relevant archive families, localized archive families, sound-bank names, expected UI package names, and Reddog debug UI assets. The local prototype extraction proved useful preview UI packages (`ui_title`, `ui_mainmenu`, `ui_loading`, `ui_worldmap`, `ui_playscreen`, `ui_playscreen_evil`, `ui_result`) plus `#SelectStage/Select.xml` route data, but the prototype build remains a secondary oracle and all preview build / toolbox / extracted payloads stay local-only and ignored.
- [x] Phase 203 on `2026-05-02`: parsed prototype `#SelectStage/Select.xml` into a repo-safe route taxonomy with `summarize_sonic_unleashed_preview_select_stage.ps1`, mapping route groups to source-recovery controller targets such as `TitleMenuController`, `SonicDayHudController`, `WerehogHudController`, `WorldMapController`, `ResultScreenController`, `AudioCueCatalog`, and `SequenceRouteController`. The taxonomy is explicitly a `prototype-only secondary oracle`; loading and pause remain `not-present-in-select-xml`, retail runtime evidence remains primary, and local Reddog assets are only a style reference for the SWARD F2 profiler panel.
- [x] Phase 204 on `2026-05-02`: promoted the prototype route taxonomy into a starter UI/UX coverage matrix for title-menu, loading, options/settings, pause, Sonic Day HUD, Werehog HUD, world-map, results, and audio/SFX. The matrix stays honest about oracle boundaries: prototype route taxonomy is secondary, retail runtime evidence remains primary, and loading/pause are runtime/package lanes rather than direct Select.xml routes. The Reddog-style F2 panel now has a compact `Coverage` tab styled through repo-safe Reddog-style helpers (`DrawSwardReddogStyleSectionHeader` / status pips) while local-only Reddog assets remain visual references and are not committed.
- [x] Phase 205 on `2026-05-02`: used the Phase 204 coverage matrix to select the next concrete source-recovery lane instead of treating every route target as equal priority; in short, coverage matrix selected Sonic Day HUD retail runtime. `summarize_sonic_unleashed_preview_select_stage.ps1` now emits `nextSourceRecoveryLane` / `sourceRecoveryLaneQueue` and reports `preview_select_stage_next_lane_status=coverage-matrix-selected-retail-sonic-day-hud`, choosing `sonic-day-hud-retail-runtime` because boost/ring-energy normalization, `SetPatternIndex` / `SetHideFlag` joins, and exact SFX/audio IDs are the remaining blockers for native 1:1 Sonic Day HUD source. The F2 `Coverage` tab mirrors the same queue through `DrawOperatorNextSourceRecoveryLane`; world-map/results prototype lanes queued for later retail runtime proof, and audio/SFX remains a parallel retail callsite lane.
- [x] Phase 206 on `2026-05-02`: returned from the prototype lane to the retail-runtime Sonic Day HUD proof matrix. `summarize_unleashed_recomp_ui_lab_hud_values.ps1` now reports `sonic_hud_runtime_proof_matrix=` and `sonic_hud_runtime_proof_matrix_status=`, keeping `gauge-scale-write`, `gauge-pattern-write`, `gauge-hide-write`, `owner-field-snapshot`, `owner-scale-correlation`, `setter-child-path-join`, and `audio-callsite` as separate auditable lanes. A capture with the core gauge/owner lanes but no audio hook reports `retail-runtime-gauge-proof-partial-audio-pending` and `sonic_hud_audio_callsite_status=audio-callsite-pending`, so boost/ring-energy can advance only when same-frame retail evidence proves the formula and exact SFX/audio IDs remain explicitly pending.
- [x] Phase 207 on `2026-05-03`: ran a real retail manual Sonic Day HUD capture through the Phase 206 matrix and found the exact blocker: owner-field snapshots, speed/tutorial semantic binding, visible gauge draw paths, rolling-counter `SetText`, and raw `SetScale` / `SetPatternIndex` / `SetHideFlag` candidates are present, but resolved gauge-write and owner-scale-correlation lanes remain empty because the setter node still does not resolve to the exact gauge child path. The runtime now emits `sonic-hud-gauge-setter-owner-candidate-correlated` for unresolved CSD setter candidates (`SetText`, `SetScale`, `SetPatternIndex`, or `SetHideFlag`) that line up with the fresh `CHudSonicStage` owner-field cache, and the summarizer reports `owner-setter-candidate-correlation` plus `sonic_hud_owner_setter_candidate_correlation_status=retail-runtime-setter-owner-candidate-correlation-pending-exact-child-path`. This unresolved setter owner-candidate correlation proves the candidate owner correlation lane without pretending it is a final 1:1 boost/ring-energy formula or exact SFX/audio proof.
- [x] Phase 208 on `2026-05-03`: launched the correct retail `UnleashedRecomp` manual runtime against `phase208_manual_20260503_130830` and confirmed the Phase 207 event fires in live Sonic Day gameplay. The upgraded summarizer now emits `owner_setter_candidate_correlation_groups=` so the raw `sonic-hud-gauge-setter-owner-candidate-correlated` rows become owner-setter candidate correlation groups by setter node, kind, semantic path/value candidate, owner address, setter values, owner-field value spans, frame span, and frame delta. The live proof matrix reached `owner-setter-candidate-correlation:present` with owner-field snapshots and rolling-counter text candidates, but exact child-path joins, direct owner-scale correlation, boost/ring-energy formula proof, and exact SFX/audio callsites remain pending. This keeps the next formula beat evidence-driven: use the grouped same-frame owner fields and setter values to narrow candidates, but do not promote final controller values until multiple-frame proof exists.
- [x] Phase 209 on `2026-05-03`: added numeric triage for those grouped retail rows without calling it a formula. `summarize_unleashed_recomp_ui_lab_hud_values.ps1` now reports `owner_setter_candidate_numeric_relation_groups=` as owner-setter candidate numeric relation groups, comparing numeric setter values against each cached owner-field offset and summarizing pair count, exact numeric matches, min/max absolute delta, setter range, owner-field range, and frame span. This is useful for spotting candidate lanes such as text values that numerically meet `ownerField480`, but it remains below proof status until the exact child path, multi-frame boost/ring-energy normalization, direct SetScale fill path, and SFX/audio callsites are proven.
- [x] Phase 210 on `2026-05-03`: added a local CT evidence lane for the user-provided Cheat Engine table without committing the table itself. `summarize_sonic_unleashed_cheat_table.ps1` parses local `CheatEngineTableVersion` XML, extracts `aobscanmodule` anchors, old `UnleashedRecomp.exe+...` injection hints, simple script constants, scans the rebuilt runtime EXE for matching byte patterns, maps hits to PE RVA/VA when available, and optionally symbolicates them with `llvm-symbolizer`. The local Phase 210 run found `29` AOB anchors and symbolized `27` runtime hits, including HUD-relevant gameplay writers for rings (`sub_82519FE8`), lives (`sub_82BDBA20` / `sub_82BDBA60`), Day boost candidates (`sub_82A50838`, plus a likely collision at `sub_82FE41C0`), Night unleash (`sub_8281FAA0`), and Tornado energy (`sub_82B11D18`). CT file stays local-only; this lane anchors gameplay value writers for HUD source recovery, but it is not UI scene/tree proof and does not promote final boost/ring-energy formulas by itself.
- [x] Phase 211 on `2026-05-03`: promoted the Phase 210 local CT evidence into retail runtime hooks for the HUD-relevant Day gameplay writers. `CHudSonicStage_patches.cpp` now wraps the CT-anchored rings writer `sub_82519FE8`, Day boost writer `sub_82A50838`, and lives/status writers `sub_82BDBA20` / `sub_82BDBA60`, emitting `sonic-hud-ct-gameplay-writer` events through `OnSonicHudCtGameplayWriter` with value name, callsite, storage address, previous/current raw value, delta, and float payload for boost when those writers execute. The HUD summarizer now reports `ct_gameplay_writer_groups=`, joins same-frame CT writer evidence against unresolved owner/setter candidate rows as `ct_gameplay_writer_owner_setter_candidate_correlation_groups=`, and adds a `ct-gameplay-writer` proof-matrix lane. Live `phase211_manual_20260503_140632` proved the fresh runtime build, `sonic-hud-ready`, exact gauge draw paths, owner-field snapshots, and `882` owner/setter candidate correlations, but still reported `ct_gameplay_writer_events=0`; next beat should exercise real ring/boost/lives mutations or add adjacent CT reader/initializer probes before claiming CT writer evidence is present. This remains a ct-anchored gameplay writer lane, not a final boost/ring-energy UI formula or audio-ID proof.
- [x] Phase 212 on `2026-05-03`: inspected the full CT shape and fixed the local CT summarizer so Cheat Engine `<CheatCodes><CodeEntry>` host instruction windows are preserved as first-class repo-safe evidence. `summarize_sonic_unleashed_cheat_table.ps1` now reports `ct_code_entries=`, `ct_host_native_sites=`, and JSON `codeEntries` rows with `entryKind=ce-code-entry`, old `UnleashedRecomp.exe+...` address strings, before/actual/after byte windows, runtime hit RVAs/VAs, and symbolicated generated-PPC source when available. The local full-table run found `29` Auto Assembler AOB anchors plus `36` CodeEntry windows (`65` host-native sites total). This proved why Phase 211 needed more caution: some CodeEntry windows are very generic and map to many functions, while the Day boost Auto Assembler AOB currently resolves to `sub_82FE41C0` in the rebuilt runtime and the Day boost CodeEntry windows include `sub_82A50838` among many hits. CT evidence remains useful, but host-site mappings must be treated as candidate runtime archaeology until live events fire and correlate against owner/setter HUD lanes.
- [x] Phase 213 on `2026-05-03`: added old CT RVA alignment reporting so the same CT can be tested honestly against the retail binary layout and the patched UI Lab binary layout. `summarize_sonic_unleashed_cheat_table.ps1` now emits `old_ct_injection_point_rva_matches=` plus per-entry `old_point_rva_status=` / `old_point_rva_matches=` and JSON `oldInjectionPointRuntimeRvaStatus`, proving when a CT `UnleashedRecomp.exe+...` address still lands exactly in the scanned runtime. Local comparison showed the clean retail install has full AOB coverage (`29/29`) and exact old-RVA hits including Day boost at `+A74E4A`, while the current patched UI Lab sidecar/build has shifted host code (`21/29` AOB anchors hit, zero exact old-RVA matches). This means the CT works for its retail target; UI Lab must use AOB-to-generated-PPC mapping and live correlation rather than assuming Cheat Engine offsets survive instrumentation unchanged.
- [x] Phase 214 on `2026-05-03`: fixed the live oracle labeling problem that made route attempts look like Sonic HUD captures while the runtime was still in a title/stage-title/item-result screen. `ui_lab_patches.cpp` now reports requested target separately from dynamic observed runtime screen fields (`observedScreen`, `observedScreenLabel`, `observedCsdProject`, `observedSourceFamily`, `observedScreenSource`, and `targetObservedMismatch`) across live bridge state, route status, UI oracle, draw-list, capture, backend, GPU, and material-correlation JSON. The classifier maps observed CSD projects such as `ui_title`, `ui_loading`, `ui_playscreen`, `ui_pause`, `ui_result`, `ui_status`, `ui_itemresult`, and `ui_worldmap` to reusable source-family names, while keeping unresolved CSDs explicit. The live `phase214_route_20260503_152518` run proved the bug: requested target was `sonic-hud`, but observed CSD/runtime evidence reported `ui_itemresult` plus title/stage shell context and no HUD value events. Future automation should gate HUD harvesting on `observedScreen=sonic-hud` or an accepted HUD family, not on the requested target alone.
- [x] Phase 215 on `2026-05-03`: tightened the dynamic observed-screen classifier after a live `phase215_observed_screen_20260503_153946` sanity launch showed another false active-screen candidate: the runtime had title-loop evidence, but the latest generic CSD creation was `ui_saveicon`, an auxiliary/passive project. `BuildObservedRuntimeScreen` now prefers live loading/title/stage/target-readiness inspectors, requires stage-target context before reporting stage HUD targets, and skips passive preload projects (`ui_saveicon`, `ui_general`, `ui_gate`, `ui_help`, `ui_balloon`, `ui_shop`, `ui_townscreen`, `ui_missionscreen`, `ui_misson`, `ui_start`) when deriving the active observed screen from CSD creation. This keeps `lastCsdProject` available as raw archaeology while preventing preload/overlay CSDs from becoming the active runtime screen.
- [x] Phase 216 on `2026-05-03`: ran a fresh CT/HUD manual route and found the next capture-gating bug before changing any formulas. The live `phase216_ct_hud_manual_20260503_154951` state correctly refused to call `ui_itemresult` a Sonic HUD session while `target=sonic-hud`, then later reached `sonic-hud-ready` with `targetCsdObserved=true` but still reported `observedScreen=unknown-csd-runtime` because the passive overlay `ui_itembox` was the latest CSD project. `BuildObservedRuntimeScreen` now treats `ui_itembox`, `ui_qte`, `ui_lcursor`, and `ui_lcursor_enemy` as passive overlay projects and lets `g_loggedStageTargetReady` plus `g_targetCsdObserved` keep the observed screen on the requested stage HUD when the latest CSD is passive or the target primary project. The same evidence with a live draw-list already shows `owner_setter_candidate_correlations=2016`, exact gauge draw paths for `so_speed_gauge` and `so_ringenagy_gauge`, and `owner_field_draw_path_bridge_groups=...`, but still reports `ct_gameplay_writer_events=0`; therefore CT-assisted writer proof remains pending and the next beat should add or correct adjacent CT-mapped writer/read probes before promoting ring/boost/lives source formulas.
- [x] Phase 217 on `2026-05-03`: added CT-anchored gameplay writer probes so live evidence can distinguish `hook not entered` from `hook entered but writer storage/mutation not proven`. `CHudSonicStage_patches.cpp` now emits `sonic-hud-ct-gameplay-writer-probe` at entry for the existing rings/lives/Day boost candidate wrappers and adds the CT AOB Day boost candidate wrapper `sub_82FE41C0` beside the broader CodeEntry candidate `sub_82A50838`. The HUD summarizer reports `ct_gameplay_writer_probe_events=` and `ct_gameplay_writer_probe_groups=` separately from `ct_gameplay_writer_events=`, and a probe-only capture sets `sonic_hud_ct_gameplay_writer_status=ct-anchored-gameplay-writer-probe-evidence-present-writer-mutation-pending` without marking the proof-matrix `ct-gameplay-writer` lane present. This is runtime archaeology for fixing the CT hook map, not final ring/boost/lives HUD source formula proof.
- [x] Phase 218 on `2026-05-03`: ran a fresh CT-assisted Sonic HUD capture on build `v1.0.3.9012970-main` using `phase218_ct_assisted_manual_20260503_161419`. The live bridge reached `observedScreen=sonic-hud`, `stageReadyEvent=sonic-hud-ready`, and `targetCsdObserved=true`; gameplay input exercised HUD-side rolling/display lanes and raised `owner_setter_candidate_correlations=1746`, `rolling_counter_semantic_groups` for `ui_playscreen/so_speed_gauge` and `ui_playscreen/so_ringenagy_gauge`, and owner-field numeric relation candidates with `field+480` as the strongest same-frame text relation. Re-running the local CT summarizer against the clean retail EXE and the patched UI Lab sidecar confirmed the useful sidecar AOB mappings: rings map to `__imp__sub_82519FE8`, Day boost script AOB maps to `__imp__sub_82FE41C0`, and lives map to `__imp__sub_82BDBA20` / `__imp__sub_82BDBA60`. The same live session then produced `ct_gameplay_writer_probe_events=1` and `ct_gameplay_writer_events=1` for `ringCount:sub_82519FE8` at frame `18713`, with the ring writer lane promoted to `ct-gameplay-writer:present:1`; boost/lives writer proof and direct CT-to-HUD owner/setter correlation remain pending. Do not promote boost/ring-energy/lives formulas from this evidence; the next proof beat needs boost/life mutations or adjacent CT reader/transition probes that fire near the active HUD setters.
- [x] Phase 219 on `2026-05-03`: added candidate-only probes for the CT CodeEntry float-gauge candidates around the old Day boost `A74BD6` window. The runtime now wraps `sub_8231C590`, `sub_8231C5F0`, and `sub_8231C628` and emits `sonic-hud-ct-code-entry-gauge-transition-candidate` rows with owner/storage addresses, raw float payloads, and input/ratio samples. The HUD summarizer reports `ct_code_entry_gauge_transition_candidate_events=`, `ct_code_entry_gauge_transition_candidate_groups=`, and `sonic_hud_ct_code_entry_gauge_transition_candidate_status=ct-code-entry-gauge-transition-candidate-present-pending-exact-value-identity` separately from `ct_gameplay_writer_events=` and the `ct-gameplay-writer` proof-matrix lane, so ambiguous CodeEntry float-gauge candidates can be observed without promoting final boost/ring-energy formulas.
- [x] Phase 220 on `2026-05-03`: ran the fresh CT-assisted manual HUD capture on build `v1.0.3.84298d1-main` under `phase219_ct_code_entry_manual_20260503_163819`. The live route reached `observedScreen=sonic-hud`, `sonic-hud-ready`, and a HUD draw-list with exact visible gauge paths for `ui_playscreen/so_speed_gauge/position/speed_gauge_color` and `ui_playscreen/so_ringenagy_gauge/position/ringenagy_gauge_color`. The new CodeEntry candidate lane fired strongly (`ct_code_entry_gauge_transition_candidate_events=5238`: `sub_8231C5F0` ratio-read `2330`, `sub_8231C628` add-clamp `2008`, `sub_8231C590` add-clamp `900`), while the normal proof lanes still keep it candidate-only. The same session produced `owner_setter_candidate_correlations=1926`, rolling-counter semantic groups for boost/ring-energy (`573` each), `ct_gameplay_writer_probe_events=1`, and `ct_gameplay_writer_events=1` for `ringCount:sub_82519FE8` at frame `9686`. Boost/lives writer proof, direct CT-to-HUD owner/setter correlation, exact child-path joins, exact boost/ring-energy formula, and audio callsites remain pending; the later input sweep left Sonic HUD for `world-map-help`, so the retained HUD draw-list snapshot is the valid gauge-path oracle for this summary.
- [x] Phase 221 on `2026-05-03`: added the missing post-processing join between CT CodeEntry gauge transition candidates and same-frame HUD owner/setter candidate rows. `summarize_unleashed_recomp_ui_lab_hud_values.ps1` now records CT CodeEntry gauge candidate rows, joins them within the existing 60-frame runtime window against matching boost/ring-energy owner/setter candidates, and reports `ct_code_entry_gauge_transition_owner_setter_candidate_correlation_groups=` with the CT callsite/phase, setter node/kind/path, `field+480`, frame delta, CT float/input range, rolling-counter setter value range, owner-field range, and joined frame span. The new `sonic_hud_ct_code_entry_gauge_transition_correlation_status=ct-code-entry-gauge-transition-owner-setter-candidate-correlation-present-pending-formula-proof` is still candidate-only: it proves the CT float transition lane co-occurs with HUD rolling-counter owner/setter evidence, but it does not promote a final boost/ring-energy formula, direct gauge fill value, child-path join, or audio proof.
- [x] Phase 222 on `2026-05-03`: added the static reverse-engineering lane on top of the CT/HUD runtime evidence without moving the local UnleashedRecomp checkout out of the project. The UI Lab build helper now uses the `W:` SUBST drive only as a transient short-path build alias unless `-KeepSubstDrive` is requested, while capture/manual helpers default to the real in-project executable path under `local_build_env\ur103clean\b\ui_lab_runtime\UnleashedRecomp\UnleashedRecomp.exe`. Added `SwardExportFunctionContext.java` plus `export_unleashed_recomp_ghidra_context.ps1` so Ghidra headless can export `static_function_context_targets=` metadata around CT/HUD candidates (`sub_824D6C18`, `sub_8231C590`, `sub_8231C5F0`, `sub_8231C628`, writer hooks, and CSD setters) into ignored `out\ui_lab_static_re\ghidra\...` JSON. The HUD summarizer now accepts `-StaticContextPath` and reports `static_runtime_correlation_groups=` plus `sonic_hud_static_context_correlation_status=static-ghidra-context-correlated-with-runtime-hud-evidence` when exported callers/callees line up with runtime CT CodeEntry, owner/setter, and rolling-counter evidence. This is static context correlation, not a final boost/ring-energy formula or leaked source claim.
- [x] Phase 223 on `2026-05-03`: ran the full Ghidra export without `-NoAnalysis`, then used the analyzed database for a native CT PE-address pass after the generated-PPC default target pass showed the stripped native address-space mismatch. The native pass resolved `12/13` targets and the HUD summarizer now maps runtime CT source aliases to native static targets, including `day-boost-a74bd6` -> `0x140A74BD6`, `0x140A74C79`, `0x140A74E4A` (`FUN_140a73d90`, caller `FUN_1416c0450`, `42` callees) and `rings` -> `0x14051FE7A` (`FUN_14051f900`, caller `FUN_14051f660`, `2` callees). The strongest next hook lane is the Day boost native `FUN_140a73d90` function around the A74BD6/A74C79/A74E4A CT sites, correlated against `sub_8231C5F0`, `sub_8231C590`, `sub_8231C628`, owner field `+480`, and rolling-counter text groups; formula proof remains pending.
- [x] Phase 224 on `2026-05-03`: added the native Day boost chain lane requested after the full Ghidra pass. The runtime emits `sonic-hud-native-ct-day-boost-site` from the generated-PPC boost candidates, tagging `sub_8231C5F0`, `sub_8231C590`, and `sub_8231C628` with native `FUN_140a73d90` CT-site identities `0x140A74BD6`, `0x140A74C79`, and `0x140A74E4A`. The summarizer now reports `native_ct_day_boost_site_events=`, `native_ct_day_boost_site_groups=`, `native_ct_day_boost_hud_chain_groups=`, and `sonic_hud_native_ct_day_boost_chain_status=native-ct-day-boost-chain-present-pending-formula-proof` when those native-site rows join rolling-counter SetText owner/setter rows through field +480. This is stronger static/native/generated-PPC/HUD source-recovery proof, not final boost/ring-energy formula proof, direct fill-scale proof, exact child-path proof, or audio ID proof.
