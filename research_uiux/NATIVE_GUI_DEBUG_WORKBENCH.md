...
- It now has Asset View plus atlas-sheet gallery navigation, CSD element navigation, selected-element crop previews, selected cast/subimage descriptors, source/destination draw-command descriptors, and mini render-target plan previews, but this is not yet a full DDS/subimage browser.
...
- It now emits authored CSD cast transform, keyframe curve, keyframe sample, sampled transform, sampled draw-command, sampled channel-state, sampled channel-evaluation descriptors, navigable Asset View CSD element bindings, selected-element crop descriptors, selected cast/subimage draw descriptors, selected source/destination draw-command descriptors, and selected render-target plans for a curated exact/proxy sample set, but it still does not drive full CSD animation playback.
...
2. Feed selected render-target plans into actual decoded subimage blits for one family-specific render pass.
