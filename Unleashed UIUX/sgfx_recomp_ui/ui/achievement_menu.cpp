// DECOUPLED from UnleashedRecomp/ui/achievement_menu.cpp — game runtime (api/SWA.h
// input state, kernel/xdbf achievement DB, user/achievement_manager, hid, locale,
// aspect patches, config, app, exports, decompressor, embedded trophy DDS) -> the
// shim + render contract. ImFontAtlasSnapshot::GetFont, Config::*, g_aspectRatioOffset*
// keep their recomp names (shim provides them); the trophy sheet loads via
// render::LoadUISprite. Body byte-identical to the recomp.
...
static std::unique_ptr<sgfx::render::Texture> g_upTrophyIcon;
...
void AchievementMenu::Init()
{
    ...
    g_upTrophyIcon = sgfx::render::LoadUISprite(sgfx::render::UISprite::Trophy);
}
