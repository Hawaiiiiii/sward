// DECOUPLED from UnleashedRecomp/ui/imgui_utils.cpp. The game runtime includes
// (aspect_ratio_patches / app / decompressor / version / res/images embedded sprites)
// are replaced by the lib's shim + render contract; the 3 UI sprites now load via the
// host (render::LoadUISprite) instead of the embedded zstd DDS. EVERYTHING ELSE in
// this file — gradients, text, easing, the pause/selection/toggle widgets — is the
// recomp's code, byte-for-byte (Config::/g_aspectRatioScale/g_versionString resolve
// to the shim under the same names).
#include "imgui_utils.h"
#include "../platform/sgfx_platform.h"
#include <imgui_internal.h>   // ImTextCharFromUtf8 / ImCharIsBlankA / ImMax (recomp pulled this in transitively)
#include <cstring>
#include <algorithm>
#include <cfloat>

std::unique_ptr<sgfx::render::Texture> g_texGeneralWindow;
std::unique_ptr<sgfx::render::Texture> g_texLight;
std::unique_ptr<sgfx::render::Texture> g_texSelect;

void InitImGuiUtils()
{
    g_texGeneralWindow = sgfx::render::LoadUISprite(sgfx::render::UISprite::GeneralWindow);
    g_texLight         = sgfx::render::LoadUISprite(sgfx::render::UISprite::Light);
    g_texSelect        = sgfx::render::LoadUISprite(sgfx::render::UISprite::Select);
}
