...
| Clean SU UI asset renderer | Separate native viewer path now exists, now opening on a full-screen local DDS-backed Loading composition instead of a single cropped cast | `b/rr89/sward_su_ui_asset_renderer.exe`; `5` screen samples, `8` local DDS-backed blits |
...
- The source-path bridge layer now includes:
  ...
  - a separate clean asset-backed renderer at `b/rr89/sward_su_ui_asset_renderer.exe`, currently smoke-guarding a full-screen Loading composition plus Main Menu, Title, and Sonic HUD DDS-backed samples
...
- now materially stronger for product-facing UI renderer direction, with `b/rr89/sward_su_ui_asset_renderer.exe` separating clean asset-backed screen rendering from the archaeology/debug workbench and opening on a full-screen Loading composition instead of a debug crop
...
> [!NOTE]
> After Phase 89, the current bottleneck is no longer just "make an executable" or "show a visual panel." The debug workbench explains the evidence chain, and the separate clean renderer now opens on a full-screen asset-backed composition instead of one isolated cast. The remaining product gap is widening those seed blits into family-specific multi-cast texture compositing/playback backed by decoded node transforms, original animation channels, exact HUD payloads, and translated host behavior.
