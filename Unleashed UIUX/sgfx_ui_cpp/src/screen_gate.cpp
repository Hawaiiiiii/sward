...
void DrawLiveValues(float panT, bool csdBase) {
    ...
    char buf[16];
    if (csdBase) {
        // HIGH SCORE / BEST TIME read clean (info_1/info_2 have no CSD slash) — single
        // values right-aligned to the shared column.
        const float DEN_R = 663.0f;
        ChromeRight(DEN_R, 328, 26.0f, act.hiScore, panT);       // HIGH SCORE (info_1)
        ChromeRight(DEN_R, 378, 26.0f, act.bestTime, panT);      // BEST TIME (info_2)
        // The CSD's RINGS/MEDALS value slots (info_3..5 num_nume/slash/num_deno) render
        // bunched on two baselines with stray/orphan "/" glyphs that don't track the medal
        // icons. Cover that sub-column with the panel's own grey gradient, then draw clean
        // single-line "X / Y" strings seated on the medal-icon rows (red sun ~y428, blue
        // moon ~y478) — one slash each, no orphan.
        DrawVGradient({ 508, 404 }, { 672, 508 },
                      WithAlpha(RGBA(114, 115, 116, 255), panT),
                      WithAlpha(RGBA(101, 102, 104, 255), panT));
        snprintf(buf, sizeof buf, "%d / %d", act.sun,  act.sunMax);
        ChromeRight(DEN_R, 412, 26.0f, buf, panT);               // sun medal row
        snprintf(buf, sizeof buf, "%d / %d", act.moon, act.moonMax);
        ChromeRight(DEN_R, 462, 26.0f, buf, panT);               // moon medal row
        PopTransform();
        return;
    }
