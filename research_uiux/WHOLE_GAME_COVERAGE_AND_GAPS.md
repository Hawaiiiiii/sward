...
| Clean SU UI asset renderer | Separate native viewer path now has visible `Prev` / `Next` navigation, screen-index labeling, no-window catalog smoke, and a full-screen local DDS-backed Loading composition seed | `b/rr90/sward_su_ui_asset_renderer.exe`; `5` screen samples, `8` local DDS-backed blits, `3` visible controls |
...
- The source-path bridge layer now includes:
  ...
  - a separate clean asset-backed renderer at `b/rr90/sward_su_ui_asset_renderer.exe`, currently smoke-guarding a visible navigation shell plus a full-screen Loading composition and Main Menu, Title, and Sonic HUD DDS-backed samples
...
- now materially stronger for product-facing UI renderer direction, with `b/rr90/sward_su_ui_asset_renderer.exe` separating clean asset-backed screen rendering from the archaeology/debug workbench, opening with visible navigation controls, and exposing no-window renderer catalog smoke
...
> [!NOTE]
> After Phase 90, the current bottleneck is no longer just "make an executable" or "show a visual panel." The debug workbench explains the evidence chain, and the separate clean renderer now has visible navigation over a smoke-guarded local DDS-backed screen catalog. The remaining product gap is widening those seed blits into family-specific multi-cast texture compositing/playback backed by decoded node transforms, original animation channels, exact HUD payloads, and translated host behavior.
