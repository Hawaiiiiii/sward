...
| Real-runtime UnleashedRecomp UI Lab parity lane | Title/menu/loading direct-context routing is evidence-backed; normal Sonic HUD now observes real stage context and binds the real `ui_playscreen` CSD project; `ui_prov_playscreen` is correctly split into an Extra/Tornado-family target but still needs deterministic owner selection | `W:\b\ui_lab_runtime\UnleashedRecomp\UnleashedRecomp.exe`; local evidence `20260427_074626` |
...
- The source-path bridge layer now includes:
  ...
  - a real-runtime UI Lab path inside the generated UnleashedRecomp build at `W:\b\ui_lab_runtime\UnleashedRecomp\UnleashedRecomp.exe`, with visible overlay controls, title/menu/loading route forcing, proven direct title-menu CSD-completion forcing, owner-output loading routing, stage-context observation, JSONL evidence logging, and a local-only screenshot/event capture helper
...
- `100%` complete against the tracked research plan through Phase `103`
...
- now materially stronger for real-runtime UI parity work, with the generated UnleashedRecomp UI Lab able to route title/menu/loading targets through live translated runtime states, prove direct title-menu entry through the real CSD completion byte, keep owner-output routing isolated to loading/stage paths, emit frame/state/loading/CSD JSONL evidence, capture local-only screenshots and observation snapshots, keep manual operator sessions alive, expose selected routes from an in-game overlay, run a passive manual observer mode that leaves normal game flow intact while still recording evidence, and bind the normal Sonic HUD route to the real `ui_playscreen` CSD project after `CGameModeStage::ExitLoading`
...
2. Keep advancing the real-runtime UI Lab instead of spending parity time on standalone approximations: native/backbuffer capture and deterministic stage/HUD owner selection are the immediate blockers now that title-menu and loading direct-context routes are evidence-backed.
...
> [!NOTE]
> After Phase 103, the clean renderer is explicitly a diagnostic sidecar. The parity lane is the generated UnleashedRecomp UI Lab because it runs the real game renderer, CSD/material/movie substrate, local game files, and translated runtime states. Title-menu direct-context entry is backed by CSD completion evidence, loading/stage-required routing still uses the owner-output bridge deliberately, normal Sonic HUD now binds `ui_playscreen` through the real runtime, and the remaining gameplay-HUD product gap is deterministic Extra/Tornado owner selection for `ui_prov_playscreen` plus native frame capture.
