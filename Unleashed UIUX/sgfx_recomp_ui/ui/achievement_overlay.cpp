// DECOUPLED from UnleashedRecomp/ui/achievement_overlay.cpp — game runtime
// (imgui_snapshot / gpu/video / kernel/memory / kernel/xdbf / locale / achievement_data /
// config / app / exports / decompressor / inspire_patches) -> the shim + render contract +
// imgui_utils. ImFontAtlasSnapshot::GetFont keeps its recomp name (font "FOT-SeuratPro-M.otf");
// the achievement DB surface (Achievement / g_xdbfWrapper / g_xdbfTextureCache / EXDBFLanguage /
// xdbf::FixInvalidSequences), Config::Language / ELanguage, Localise and Game_PlaySound keep
// their recomp names (shim provides them). The CanDequeueAchievement() audio-volume probe and
// the PPC_FUNC sound-administrator hook are HARD kernel/PPC game-state coupling — see
// gameStateCoupling; minimally stubbed with SGFX-TODO so the body structure stays intact.
...
// SGFX-TODO [gameStateCoupling]: PPC_FUNC hook over the recompiled guest sound-administrator
// update (sub_82B43480) that sets g_soundAdministratorUpdated. This is a kernel-level PPC
// function trampoline (kernel/function.h) with no shim equivalent; the host must signal
// "audio is alive this frame" through its own hook. Removed here so the lib links standalone.
...
static bool CanDequeueAchievement()
{
    ...
    {
        // SGFX-TODO [gameStateCoupling]: the recomp probes the guest audio center
        // (g_memory.Translate of 0x83362FFC, be<uint32_t> byte-swap reads, InspirePatches
        // scene-name category select) to ensure non-zero volume before playing the unlock
        // cue. That is kernel guest-memory inspection with no shim equivalent — host decides
        // whether audio is playable. Stubbed to "playable" so queued achievements still show.
        return true;
        /*
